...
\begin{abstract}
  ...
  parallel with the continuous-time Ramsey tax problem studied in
  \citet{JiangSargentWang2024}.  In the Ramsey problem, decisions are ordered by
...
Following \citet{JiangSargentWang2024}, hereafter JSW, consider a continuous-time version of a special case of the \citet{LucasStokey1983} economy in which there is no initial term debt to be financed.  A representative household and a
...
\end{equation}
meaning that the government carries \emph{no} initial term debt.\footnote{JSW focus on versions of Lucas and Stokey's Ramsey problems in which a non-zero schedule of initial term debts is  central. That part of their analysis is not the focus of this note and would just obscure the connections that we focus on here.  JSW altered the timing protocol relative to \citet{LucasStokey1983} in order to overcome the difficulty identified by \citet{debortoli2021optimal}.  \citet{JiangSargentWang2024} is a continuous-time working paper; a discrete-time companion that incorporates the timing-protocol change but not the dynamic program is forthcoming in the \emph{Journal of Political Economy} as \citet{JiangSargentWang2025} We retain JSW's timing protocol here.}
...
\eqref{eq:R_resource}--\eqref{eq:R_impl}. The tax rate and competitive equilibrium price system can be deduced from
the allocation.
...
\begin{remark}[Backward-looking character of $\Pi_t$]
Equation~\eqref{eq:R_Pi} defines $\Pi_t$ as an integral over past primary deficits, making it a purely backward-looking state variable: its value at time $t$ is fully determined by the history of $(G_s, \tau_s, N_s)$ for $s \le t$.
\end{remark}
...
\begin{remark}[Timing protocol and $X_t$ as a state variable]
Because $X_t = \Pi_t U_{C,t}$ depends on the current marginal utility $U_{C,t}$,
whether it is predetermined requires care.  \citet{JiangSargentWang2024} adopt
the protocol that for all $t >0$ the tax rate at time $t$ is set at $t-dt$, making $C_t$ (and
hence $U_{C,t}$) predetermined for all  $t>0$.  This makes $X_t$ a genuine state variable
for the continuation problem at $t >0$, and is the continuous time instantiation of JSW's key modification of
\citeauthor{LucasStokey1983}'s original timing protocol. 
\end{remark}

\begin{remark} (Time $0$ is different)
The Ramsey planner chooses $C_0$ at time $0$. This makes the Ramsey planner's decision problem different
from the decision problems of the ``continuation Ramsey planners'' to be introduced soon.
\end{remark}
...
\end{equation}
The optimization problem on the right side of \eqref{eq:R_HJB} is that of a {\em continuation Ramsey planner} at time $t$ who faces state variable $X_t$ and chooses $C_{t+dt}$.

\subsection{The Ramsey planner and the continuation Ramsey planners}
\label{sub:two_ramsey_planners}

Following \citet{LucasStokey1983} and the QuantEcon lectures on optimal
taxation,\footnote{See \url{https://python-advanced.quantecon.org/opt_tax_recur.html} and \url{https://python-advanced.quantecon.org/amss.html}} it is instructive to distinguish sharply between two roles in recursive implementations of a Ramsey plan, each with its own state and choice variables.

\medskip
\noindent\textbf{The Ramsey planner at $t=0$.}
The Ramsey planner operates once, at $t=0$.
\begin{itemize}
  \item \emph{State variable.} There is no initial term debt for the Ramsey planner to service;
    consequently, it faces state variable $X_0 = 0$ (see
    \eqref{eq:R_nodebt}).
  \item \emph{Choice variables.}  (i) The time-$0$ allocation $(C_0, N_0)$;
    and (ii) the entire trajectory $\{X_s\}_{s>0}$ of promised values to be
    bequeathed to continuation planners at every future instant.  By the
    structure of the HJB these choices collapse to a single free scalar:
    the Lagrange multiplier $\Phi_0\ge 0$ on \eqref{eq:R_impl}.  Once
    $\Phi_0$ is chosen, the policy rule $C^* = C(\Phi_0; G_t)$ from
    \eqref{eq:R_Copt} pins down consumption at every $t\ge 0$.
  \item \emph{Value function.}  $ V(0,0;\Phi_0^*) = H(0;\Phi_0^*)$,
    since $X_0 = \Pi_0 U_{C,0} = 0$.
\end{itemize}

\medskip
\noindent\textbf{A continuation Ramsey planner at $t>0$.}
A continuation Ramsey planner arrives at time $t>0$ with an
inherited state.  
\begin{itemize}
  \item \emph{State variables.} $(X_t, t)$, where $X_t = \Pi_t U_{C,t}$ is
    the marginal-utility-scaled cumulative liability.  The continuation
    planner is \emph{obligated} to take $X_t$ as given.
  \item \emph{Choice variable.}  Current consumption $C_t$.  Through the law
    of motion \eqref{eq:R_dXt}, choosing $C_{t+dt}$ implies the promised value
    $X_{t+dt}$ passed to the next continuation planner.
  \item \emph{Value function.}  $V(X_t, t;\Phi_0) = -\Phi_0 X_t + H(t;\Phi_0)$.
    The shadow price $V_X = -\Phi_0$ is the \emph{same constant} for every
    continuation planner along the original Ramsey path.
\end{itemize}

\medskip
\noindent\textbf{The time-inconsistency signature.}
The two planners differ not only in their state variables but in the constraints
that determine their first-order conditions.  The time-$0$ Ramsey planner
faces initial state $X_0 = 0$ and  chooses $C_0$ and $\Phi_0$; a continuation Ramsey
planner at $t>0$ inherits the state $X_t = \Pi_t U_{C,t}$ and chooses $C_{t+ dt}$. 
%It treats $\Phi_0$ as a fixed parameter bequeathed by the time-$0$ plan.


\subsection{Linear value-function guess and verification}
...
$X_t$.  Optimal consumption satisfies
...
\medskip
\begin{remark}[Interpretation of $\Phi_0$]
...
%% ---------------------------------------------------------------------------
\subsection{Cyclical government expenditures}
\label{sub:cyclical_ramsey}
%% ---------------------------------------------------------------------------

To illustrate the power of the linear guess, we now specialise to
$U(C,N)=\ln C - N$ (the baseline utility used in JSW's quantitative
examples) and allow government spending to oscillate around a mean:
\begin{equation}\label{eq:cyc_G}
  G_t = G_0 + \theta_G \cos(\omega t), \qquad G_0,\,\theta_G>0,\;\omega>0,
\end{equation}
so that $G_t > 0$ for all $t$ provided $\theta_G < G_0$.

\subsubsection{Constant optimal tax from the HJB}
...
component of fiscal needs does not appear in the optimality condition for
consumption.  The Ramsey planner therefore imposes a \emph{constant} tax
rate at every date, even though government expenditures oscillate.  This is
the HJB incarnation of the tax-smoothing result of \citet{barro1979determination}.

\medskip
\begin{remark}[Why $G_t$ drops out]
The HJB maximand contains $G_t$ twice: once in the felicity term $-(C+G_t)$
and once in the incentive term $-(1+\Phi_0)G_t$.  Both enter additively as
constants in the maximization over $C$.  The optimum $C^*$ is therefore
pinned by the ratio of marginal utility to the Lagrange multiplier, with no
role for $G_t$.
\end{remark}

\subsubsection{Determining $\Phi_0^*$ from the implementability constraint}
...
the discounted present value of government expenditures, not on their timing.
Second, the cyclical component is attenuated by $\rho^2/(\rho^2+\omega^2)\in(0,1)$:
high-frequency cycles ($\omega\gg\rho$) contribute negligibly to the tax rate
because they nearly cancel in present value.  Third, as $\theta_G\to 0$ the
formula collapses to $1-C^* = G_0$.

\subsubsection{The cyclical state variable $X_t^*$}
...
near zero.  For $\omega \gg \rho$ the amplitude is approximately
$\theta_G/\omega$, so high-frequency spending shocks produce only small
oscillations in the state variable.

\medskip
\begin{remark}[$X_t^*$ as a fiscal buffer]
Variations in $X_t^*$ map one-for-one into variations in $\Pi_t^*$ (since
$C^*$ is constant).  When $G_t$ is high the government runs a deficit,
pushing $\Pi_t^*$ up; when $G_t$ falls back, the surplus rises and $\Pi_t^*$
declines.  The tax rate is constant throughout because it is the
\emph{state variable}, not the tax rate, that absorbs fiscal fluctuations.
\end{remark}

\subsubsection{Structure of $H(t;\, \Phi_0^*)$}

The time-varying component $H(t;\,\Phi_0^*)$ satisfies the ODE
\eqref{eq:R_Hode} with $C_t = C^*$ fixed:
\begin{equation}\label{eq:cyc_Hode}
  \rho H(t;\,\Phi_0^*)
  = H'(t;\,\Phi_0^*)
  + \ln C^* - (C^*+G_t) + \Phi_0^*\bigl[1 - (C^*+G_t)\bigr].
\end{equation}
Because $G_t$ oscillates, $H$ has the form
\begin{equation}
  H(t;\,\Phi_0^*) = H_\infty + A_H\cos(\omega t) + B_H\sin(\omega t),
\end{equation}
where $H_\infty = [\ln C^* - (1+\Phi_0^*) C^*]/\rho$ and $(A_H, B_H)$ are
determined by matching $\omega$-frequency terms.  The value function
$V(X_t^*, t) = -\Phi_0^* X_t^* + H(t;\Phi_0^*)$ oscillates both through
the state $X_t^*$ and directly through $H(t)$, together capturing the
welfare cost of the cyclical fiscal program.  The optimal control rule is
time-invariant throughout: the HJB linear guess cleanly separates the
time-varying fiscal environment (absorbed by $H$) from the constant
policy rule (pinned by $\Phi_0^*$).

%% ===========================================================================
\section{The Mirrlees Problem: A Sequential Structure in Increasing Unobserved Productivity}
\label{sec:mirrlees}
%% ===========================================================================

\subsection{Environment}

Following the lecture notes of \citet{Bhandari2026} on optimal nonlinear taxation, there is a continuum
...
to types below $z$ and now faces types $\zeta > z$ with
...
\medskip\noindent\textbf{Sequential decision-making protocol and promised values.}
...
$\zeta < z$.  A continuation planner starting at $z$ is obliged to
deliver exactly $u$ to type $z$.  Types already processed ($\zeta < z$) have
received their allocations and cannot be changed.  This is the Mirrlees analogue of the commitment structure in the Ramsey problem: just as $X_t = \Pi_t U_{C,t}$ records the cumulative fiscal obligation inherited by each continuation Ramsey planner, the promised utility $u$ records the incentive obligation inherited by each continuation Mirrlees planner.

\begin{remark} (Productivity $\zlow$ is different)
The Mirrlees planner at $z = \zlow$ chooses $y(\zlow)$ and the initial promised
utility $u(\zlow)$.  This makes the $\zlow$-planner's decision problem different
from those of the ``continuation Mirrlees planners'' to be introduced soon.
\end{remark}

\subsection{The Mirrlees planner and the continuation Mirrlees planners}
\label{sub:two_mirrlees_planners}

Paralleling the Ramsey treatment in Section~\ref{sub:two_ramsey_planners}, we
distinguish the Mirrlees planner at the bottom of the type distribution from
all continuation Mirrlees planners.

\medskip
\noindent\textbf{The Mirrlees planner at $z=\zlow$.}
The planner at $\zlow$ faces no inherited commitment.
\begin{itemize}
  \item \emph{State variable.}  The initial promised utility $u(\zlow)$, which
    is a \emph{free endpoint} in problem~\eqref{eq:M_obj}; no prior type has
    imposed an obligation.  The participation constraint pins a floor on
    $u(\zlow)$, playing the role that the initial government debt plays in the
    general Ramsey problem (set to zero here under \eqref{eq:R_nodebt}).
  \item \emph{Choice variables.}  (i) The output $y(\zlow)$ for the lowest
    type; and (ii) the entire schedule $\{\calU(\zeta)\}_{\zeta>\zlow}$ of
    promised utilities to bequeath to continuation planners.  By the structure
    of the HJB these choices collapse to the single scalar $\mu\ge 0$ (the
    Lagrange multiplier on the resource constraint \eqref{eq:M_RC}): once
    $\mu$ is set, the FOC \eqref{eq:M_ystar} pins down output at every type.
  \item \emph{Value function.} $V(u(\zlow),\zlow;\mu) = \lambda(\zlow)\,u(\zlow)
    + K(\zlow;\mu)$.  The boundary condition $\lambda(\zlow)=0$ means the
    $\zlow$-planner's value reduces to $K(\zlow;\mu)$, the aggregate resource
    surplus from the entire mechanism.
\end{itemize}

\medskip
\noindent\textbf{The continuation Mirrlees planner at $z>\zlow$.}
A continuation Mirrlees planner at $z$ arrives with an inherited
state.
\begin{itemize}
  \item \emph{State variables.}  $(u, z)$, where $u = \calU(z)$ is the
    promised utility inherited from all types below $z$.  The continuation
    planner is \emph{obligated} to honor $u$ and cannot renegotiate it.
  \item \emph{Choice variable.}  The output $y(z)$ for type $z$.  Through the
    IC law of motion \eqref{eq:M_IC2}, choosing $y(z)$ implies the promised
    utility $\calU(z+dz)$ passed to the next continuation planner.
  \item \emph{Value function.}  $V(u, z; \mu) = \lambda(z)\, u + K(z;\mu)$.
    The shadow price $V_u = \lambda(z)$ records the social cost of tightening
    incentive constraints at type $z$.
\end{itemize}

\medskip
\noindent\textbf{Constant versus varying shadow prices.}
A key difference from the Ramsey problem is that the Mirrlees shadow price
$V_u = \lambda(z)$ \emph{varies} along the type distribution, whereas the
Ramsey shadow price $-V_X = \Phi_0$ is \emph{constant} across all
continuation planners.  The Mirrlees costate $\lambda(z)$ satisfies the ODE
\eqref{eq:M_costate} and changes because the redistribution pressure shifts
as the mechanism moves through the type distribution: $\lambda(z)$ is
typically hump-shaped, large (in absolute value) where the planner most
values relaxing the incentive constraint, and zero at both boundaries.  The
constancy of $\Phi_0$ in Ramsey reflects the homogeneity of fiscal policy
across time under commitment; the variation of $\lambda(z)$ in Mirrlees
reflects the heterogeneity of incentive costs across types.
...
\end{equation}
This is the \citet{diamond1998optimal} ABC formula.
...
\begin{remark}[Initial planners versus continuation planners]
\label{rem:t0_special}
The table records a further structural parallel: the special role of the
\emph{initial} planner in each problem.

\medskip
\noindent\textbf{Ramsey.}
The entry ``Initial condition: $X_0 = 0$'' in the table is more than a
technicality.  The entire Ramsey program is determined at time $t=0$ by the
single number $\Phi_0^*$: once it is chosen, the continuation policy $C^*(t)$,
$N^*(t)$, and the state trajectory $X_t^*$ are pinned down for all $t > 0$.
The time-$0$ Ramsey planner is therefore in a unique position --- it is the
only planner that can set $\Phi_0$ freely (subject to the implementability
constraint \eqref{eq:R_IC}).  Every continuation Ramsey planner inherits
$X_t^* \ne 0$ and must treat $\Phi_0$ as a given parameter.  A continuation
Ramsey plan that restarts from some $(X_t, t)$ with $X_t > 0$ is
\emph{not} a Ramsey plan starting from $t$: it would choose a different
multiplier $\widetilde{\Phi}(X_t) \ne \Phi_0$, which is precisely the
time-inconsistency of the Ramsey plan.

\medskip
\noindent\textbf{Mirrlees.}
The entry ``Free endpoint: $\calU(\zlow)$ optimized; $\lambda(\zlow)=0$''
has the same structure.  The entire mechanism is determined at $z=\zlow$ by
the single number $u(\zlow)$: this initial promised utility, together with
the Envelope ODE \eqref{eq:M_IC2}, pins down the complete allocation
$\{\calU(z)\}_{z\ge\zlow}$.  The planner at $\zlow$ is the only planner
free to choose $u(\zlow)$ (subject to the participation constraint).  A
continuation Mirrlees planner that restarts at an intermediate type
$z' > \zlow$ inherits $u(z') \ne u(\zlow)$ and faces a different problem.
Commitment by the $\zlow$-planner --- to honor the utilities generated by
the mechanism for all $\zeta \in [\zlow, z']$ --- prevents such
reoptimization, exactly as commitment by the time-$0$ Ramsey planner
prevents resetting $\Phi_0$ at $t > 0$.

\medskip
\noindent\textbf{Summary.}
In both problems the initial planner simultaneously (i)~faces no inherited
state to which it is obligated, and (ii)~sets the single free parameter
($\Phi_0$ or $u(\zlow)$) that governs the entire continuation.  All
subsequent planners are bound by a state they cannot revise.  The table's
``determines $\Phi_0$: $I_0(\Phi_0)=0$'' and ``determines $\mu$: resource
constraint $= B$'' rows reflect how each problem closes: just as $B$ pins $\mu$
through the resource constraint for the Mirrlees mechanism, the implementability
constraint closes the Ramsey problem and pins $\Phi_0^*$.
\end{remark}
...
conditions derived from the Hamiltonian approach of \citet{Bhandari2026}---the
...
calculus-of-variations problem (discounted in the Ramsey problem, undiscounted in the Mirrlees problem).
...
The Hamiltonian approach of \citet{Bhandari2026} defines
...
%% ===========================================================================
\section{The Mirrlees HJB with Pareto-Distributed Productivity}
\label{app:pareto}
%% ===========================================================================
...
support.  \citet{diamond2011case} argue that the zero-top-rate result is an
...
compensated elasticity of taxable income).  \citet{saez2001using} surveys estimates
...
formula gives $T'_\infty = 1/(1+1) = 50\%$.  \citet{diamond2011case}
...
\bibliography{Mirrlees_HJB_refs}
