...
continuous-time version of a special case of a Lucas--Stokey (1983) economy in which there is no initial term debt to be financed.  A representative household and a
...
\end{equation}
We shall regard the choice problem on the right side of equation \eqref{eq:R_HJB} as confronting
a {\em continuation Ramsey planner} at time $t$ who confronts state variable $X_t$ and chooses $C_t$.

\subsection{The Ramsey planner and the continuation Ramsey planners}
\label{sub:two_ramsey_planners}

Following Lucas and Stokey (1983) and the QuantEcon lecture on optimal
taxation, it is instructive to distinguish sharply between two roles in the
Ramsey problem, each with its own state and choice variables.

\medskip
\noindent\textbf{The Ramsey planner at $t=0$.}
The Ramsey planner operates once, at $t=0$.
\begin{itemize}
  \item \emph{State variable.} There is no initial debt that the Ramsey planner is committed to service.
...
A continuation Ramsey planner arrives at time $t>0$ with a pre-committed
inherited state.  It cannot re-optimize $\Phi_0$ from scratch.
\begin{itemize}
  \item \emph{State variables.} $(X_t, t)$, where $X_t = \Pi_t U_{C,t}$ is
    ...
    planner is \emph{obligated} to take $X_t$ as given; it records the fiscal
    promises made on the household's behalf by all previous planners.
  \item \emph{Choice variable.}  Current consumption $C_t$.  Through the law
    of motion \eqref{eq:R_dXt}, choosing $C_t$ implies the promised value
...
The two planners differ not only in their state variables but in the object
that determines their first-order conditions.  The Ramsey planner at $t=0$
faces initial debt $b_0 =0$ and freely chooses $\Phi_0$; the continuation Ramsey
planner at $t>0$ faces the marginal-utility-scaled debt $X_t = u_c \cdot b_t$
and treats $\Phi_0$ as a fixed parameter inherited from the Ramsey plan.
...
received their allocations and cannot be changed.  This is the Mirrlees analogue of the Ramsey planner's commitment to
...
that continuation  Ramsey planners must finance, the promised utility $u$ in the
Mirrlees HJB records the incentive obligation to type $z$ that continuation  Mirrlees
planners must honor. 

\subsection{The Mirrlees planner and the continuation Mirrlees planners}
\label{sub:two_mirrlees_planners}

Paralleling the LS treatment, we distinguish the Mirrlees planner at the
bottom of the type distribution from all continuation Mirrlees planners.
...
\begin{itemize}
  \item \emph{State variable.}  The initial promised utility $u(\zlow)$, which
    ...
    $u(\zlow)$, playing the role that the initial debt $b_0$ plays in the
    Ramsey problem.
...
A continuation Mirrlees planner at $z$ arrives with a pre-committed inherited
state and cannot revise it.
\begin{itemize}
