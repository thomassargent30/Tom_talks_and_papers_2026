...
\paragraph{Compensated (Hicksian) ETI on the linearized schedule.}
The \textbf{linearized compensated elasticity of taxable income} is
\begin{equation}
\label{eq:ETI-lin-def}
  \zeta^c_{\mathrm{lin}}(y)
  \;:=\;
...
The \textbf{linearized uncompensated ETI} is defined analogously without
the utility compensation:
\[
  \zeta^u_{\mathrm{lin}}(y)
  \;:=\;
...
\paragraph{The deformation factor and the structural ETI.}
\label{par:deformation}

The linearized elasticity $\zeta^c_{\mathrm{lin}}$ measures the behavioral response to a marginal rate change \emph{holding the rest of the schedule fixed}---as if the agent faced a flat tax at rate $T'(y)$. On a nonlinear schedule, however, a behavioral adjustment $dy$ moves the agent to a point where the marginal rate differs by $T''(y)\,dy$, inducing a further response, and so on. The total compensated response $dy$ to an exogenous marginal rate increase $d\tau$ must satisfy the fixed-point equation
\begin{equation}
\label{eq:fixed-point}
  dy = -\frac{y\,\zeta^c_{\mathrm{lin}}}{1-T'(y)}\bigl[d\tau + T''(y)\,dy\bigr].
\end{equation}
The first term in brackets is the exogenous rate increase; the second is the endogenous rate change from sliding along the curved schedule. Solving for $dy$:
\begin{equation}
\label{eq:dy-deformation}
  dy = -\frac{y\,\zeta^c_{\mathrm{lin}}}{1-T'(y)}\cdot\frac{d\tau}{D(y)},
\end{equation}
where
\begin{equation}
\label{eq:D-def}
  \boxed{D(y) \;:=\; 1 + \frac{y\,\zeta^c_{\mathrm{lin}}(y)}{1-T'(y)}\,T''(y)}
\end{equation}
is the \textbf{deformation factor} (Golosov, Tsyvinski, and Werquin 2014). It captures the feedback between the agent's response and the curvature of the baseline schedule:
\begin{itemize}[leftmargin=2em]
  \item $T''(y) > 0$ (convex schedule, marginal rates rising): $D > 1$, the feedback is \emph{stabilizing}. When the agent reduces income, he slides into a region with a lower marginal rate, which partially offsets the incentive to reduce further. The total response is \emph{smaller} than the linearized response.
  \item $T''(y) < 0$ (concave schedule, marginal rates falling): $D < 1$, the feedback is \emph{destabilizing}. Reducing income means facing a higher marginal rate, amplifying the original response.
  \item $T''(y) = 0$ (linear baseline): $D = 1$, no feedback. The linearized and structural elasticities coincide.
\end{itemize}

We define the \textbf{structural (or total) compensated ETI} as
\begin{equation}
\label{eq:ETI-def}
  \boxed{%
  \zeta^c(y)
  \;:=\;
  \frac{\zeta^c_{\mathrm{lin}}(y)}{D(y)}
  \;=\;
  \frac{\zeta^c_{\mathrm{lin}}(y)}{1 + \dfrac{y\,\zeta^c_{\mathrm{lin}}(y)}{1-T'(y)}\,T''(y)}\,.}
\end{equation}
This is the elasticity that governs the \emph{actual} behavioral response on the nonlinear schedule: the percentage change in earnings per percentage change in the net-of-tax rate, inclusive of the feedback from schedule curvature. When $T''=0$, $\zeta^c = \zeta^c_{\mathrm{lin}}$.

\smallskip
\noindent
The \textbf{structural uncompensated ETI} and the \textbf{structural income effect} are defined analogously:
\[
  \zeta^u(y) := \frac{\zeta^u_{\mathrm{lin}}(y)}{D(y)}, \qquad
  \eta(y) := \frac{\eta_{\mathrm{lin}}(y)}{D(y)},
\]
where $\eta_{\mathrm{lin}} := -(1-T')\,\partial \hat{y}/\partial I > 0$ is the linearized income effect. The Slutsky relation $\zeta^c = \zeta^u + \eta$ continues to hold for the structural elasticities, since dividing through by $D$ preserves the identity.

\begin{remark}[Relationship to labor-supply elasticities]
Since $y=z\,n$ with $z$ fixed, $\zeta^c_{\mathrm{lin}}$ coincides with the
compensated elasticity of labor supply $\varepsilon^c$ from
Section~\ref{sec:elasticities}, and $\zeta^u_{\mathrm{lin}} = \varepsilon^u$.
Under quasilinear-in-consumption preferences ($U(c,n)=c-v(n)$),
income effects vanish and
$\zeta^c_{\mathrm{lin}}=\zeta^u_{\mathrm{lin}}=\varepsilon^f=1/\gamma$, so that $\zeta^c = \zeta^u = (1/\gamma)/D(y)$.
\end{remark}

\begin{remark}[When $D$ matters in practice]
The deformation factor $D(y)$ matters for two distinct purposes:
\begin{enumerate}[leftmargin=2em]
  \item \textbf{Characterizing the optimum.} At the optimal schedule, $T''$ is typically smooth and often small (e.g., Saez 2001 shows $T'' \to 0$ at the top under Pareto tails), so $D \approx 1$ and the distinction between linearized and structural elasticities is quantitatively minor.
  \item \textbf{Evaluating reforms of the current code.} The existing tax schedule has discrete brackets and phase-outs with large $|T''|$, so $D$ can deviate substantially from~1. In this case, using the linearized elasticity without the $D$ correction overstates (when $T''>0$) or understates (when $T''<0$) the behavioral response. See Golosov, Tsyvinski, and Werquin (2014, Proposition 1) for the general multivariate version.
\end{enumerate}
\end{remark}

%--------------------------------------------------------------------------
\paragraph{Behavioral response to an arbitrary tax perturbation.}
\label{par:arbitrary-perturbation}
%--------------------------------------------------------------------------

The results derived so far characterize the behavioral response to a pure marginal-rate change~$d\tau$ (equation~\eqref{eq:dy-deformation}).  We now derive the behavioral response to an \emph{arbitrary} perturbation of the tax schedule, which simultaneously changes both the marginal rate and the tax level at every income.  This result, which is the static one-dimensional specialization of Golosov, Tsyvinski, and Werquin (2014, Proposition~1), is the key input for the welfare analysis of tax reforms in Section~\ref{sec:perturb-setup}.

\begin{assumption}[Local Lipschitz continuity]
\label{as:lipschitz}
For every type~$z$ and every admissible perturbation $\delta T \in C^2(\R_+,\R)$, there exist $\bar\epsilon > 0$ and $M > 0$ such that
\[
  \epsilon < \bar\epsilon \implies |y(z;\,T + \epsilon\,\delta T) - y(z;\,T)| \;\le\; M\,\epsilon,
\]
where $y(z;\,\tilde T)$ denotes optimal earnings of type~$z$ under the tax schedule~$\tilde T$.
\end{assumption}

\noindent
This assumption rules out bunching and ensures that earnings respond continuously (in fact, first-order) to infinitesimal perturbations of the schedule.  It is the one-dimensional version of Golosov, Tsyvinski, and Werquin (2014, Assumption~2).

\begin{proposition}[Gateaux differential of earnings]
\label{prop:GWT-1D}
Let $T \in C^2(\R_+,\R)$ be a baseline schedule with $T'(y) \in (0,1)$ and $D(y) > 0$ for all~$y$ in the support of the earnings distribution.  Let $\delta T \in C^2(\R_+,\R)$ be an admissible perturbation.  Suppose Assumption~\ref{as:lipschitz} holds.  Then the earnings functional $y(z;\,\cdot)$ is Gateaux differentiable at $T$ in the direction~$\delta T$, and the Gateaux differential is
\begin{equation}
\label{eq:GWT-delta-y}
  \boxed{%
  \delta y(z;\,T,\delta T)
  \;=\;
  \underbrace{-\;\frac{\zeta^c(y)\,y}{1-T'(y)}\;\delta T'(y)}_{\displaystyle\text{compensated effect}}
  \;+\;
  \underbrace{\frac{\eta(y)}{1-T'(y)}\;\delta T(y)}_{\displaystyle\text{income effect}}\,,}
\end{equation}
where $y = y(z;\,T)$ is baseline earnings, $\zeta^c(y) = \zeta^c_{\mathrm{lin}}(y)/D(y)$ is the structural compensated ETI~\eqref{eq:ETI-def}, and $\eta(y) = \eta_{\mathrm{lin}}(y)/D(y)$ is the structural income effect.

Equivalently, factoring out $1/D(y)$:
\begin{equation}
\label{eq:GWT-delta-y-lin}
  \delta y
  \;=\;
  \frac{1}{D(y)}
  \left[
    -\frac{\zeta^c_{\mathrm{lin}}(y)\,y}{1-T'(y)}\;\delta T'(y)
    \;+\;
    \frac{\eta_{\mathrm{lin}}(y)}{1-T'(y)}\;\delta T(y)
  \right].
\end{equation}
\end{proposition}

\begin{proof}
The proof proceeds in four steps: (i)~write the equilibrium condition for the perturbed schedule, (ii)~take a first-order Taylor expansion using the Lipschitz bound, (iii)~express the result in terms of the linearized Marshallian demand, and (iv)~apply the Slutsky decomposition.

\medskip
\noindent\textbf{Step 1 (Tangent hyperplane at the perturbed optimum).}
Under the baseline schedule~$T$, agent~$z$ earns $y = y(z;\,T)$ and faces the tangent linear budget
\[
  c = (1-\tau)\,\tilde y + I,
  \qquad
  \tau := T'(y),
  ...
  I := \tau\,y - T(y),
\]
with Marshallian demand $\hat y(1-\tau,\,I,\,z) = y$ (cf.\ \eqref{eq:linearized-problem-pert}).

Under the perturbed schedule $\tilde T = T + \epsilon\,\delta T$, agent~$z$ earns $\tilde y = y + \epsilon\,\delta y + o(\epsilon)$ (by Assumption~\ref{as:lipschitz}).  At $\tilde y$, the tangent linear budget has parameters:
\begin{align}
  1 - \tilde\tau
  &= 1 - T'(\tilde y) - \epsilon\,\delta T'(\tilde y)
  = (1-\tau) - T''(y)\,\epsilon\,\delta y - \epsilon\,\delta T'(y) + o(\epsilon),
  \label{eq:perturbed-ntr}\\[4pt]
  \tilde I
  &= \tilde\tau\,\tilde y - T(\tilde y) - \epsilon\,\delta T(\tilde y).
  \label{eq:perturbed-VI-def}
\end{align}
To expand~\eqref{eq:perturbed-VI-def}, write $\tilde\tau = \tau + T''(y)\,\epsilon\,\delta y + \epsilon\,\delta T'(y) + o(\epsilon)$ and $\tilde y = y + \epsilon\,\delta y + o(\epsilon)$:
\begin{align}
  \tilde I
  &= \bigl[\tau + T''(y)\,\epsilon\,\delta y + \epsilon\,\delta T'(y)\bigr](y + \epsilon\,\delta y)
     \;-\;
     \bigl[T(y) + T'(y)\,\epsilon\,\delta y + \tfrac{1}{2}T''(y)(\epsilon\,\delta y)^2\bigr]
     \;-\; \epsilon\,\delta T(y)
     \;+\; o(\epsilon) \notag\\
  &= \underbrace{\tau\,y - T(y)}_{=\,I}
     \;+\; \epsilon\Bigl[T''(y)\,y\,\delta y + \delta T'(y)\,y - \delta T(y)\Bigr]
     \;+\; o(\epsilon).
  \label{eq:perturbed-VI}
\end{align}
Hence the first-order changes in the tangent parameters are:
\begin{equation}
\label{eq:tangent-changes}
  \Delta(1-\tau) = -\epsilon\bigl[T''(y)\,\delta y + \delta T'(y)\bigr] + o(\epsilon),
  \qquad
  \Delta I = \epsilon\bigl[T''(y)\,y\,\delta y + \delta T'(y)\,y - \delta T(y)\bigr] + o(\epsilon).
\end{equation}

\medskip
\noindent\textbf{Step 2 (Marshallian demand expansion).}
Since $\tilde y = \hat y(1-\tilde\tau,\,\tilde I,\,z)$, a first-order expansion of the Marshallian demand around $(1-\tau,\,I)$ gives:
\begin{equation}
\label{eq:marshallian-expansion}
  \epsilon\,\delta y
  = \frac{\partial\hat y}{\partial(1-\tau)}\,\Delta(1-\tau)
  \;+\;
  \frac{\partial\hat y}{\partial I}\,\Delta I
  \;+\; o(\epsilon).
\end{equation}
This step relies crucially on Assumption~\ref{as:lipschitz}: it guarantees that $\tilde y - y = O(\epsilon)$, so that second-order terms in the Taylor expansion are $o(\epsilon)$.

\medskip
\noindent\textbf{Step 3 (Substitution and the Slutsky identity).}
Define the uncompensated and income-effect responses:
\[
  A := \frac{\partial\hat y}{\partial(1-\tau)}, \qquad B := \frac{\partial\hat y}{\partial I}.
\]
Substituting~\eqref{eq:tangent-changes} into~\eqref{eq:marshallian-expansion} and dividing by~$\epsilon$:
\begin{equation}
\label{eq:fixedpt-raw}
  \delta y
  = -A\bigl[T''\delta y + \delta T'(y)\bigr]
  \;+\; B\bigl[T''\,y\,\delta y + \delta T'(y)\,y - \delta T(y)\bigr].
\end{equation}
Group the terms involving the direct perturbation $\delta T$ and the feedback through $\delta y$:
\begin{equation}
\label{eq:fixedpt-grouped}
  \delta y
  = \underbrace{\bigl[-(A - B\,y)\,\delta T'(y) - B\,\delta T(y)\bigr]}_{\text{direct effect}}
  \;+\; \underbrace{(-A + B\,y)\,T''}_{\text{feedback coefficient}} \cdot\delta y.
\end{equation}
The Slutsky equation for the linearized demand (Lemma~\ref{lem:slutsky-tangent} below) states:
\begin{equation}
\label{eq:slutsky-tangent}
  A - B\,y = C, \qquad\text{where}\quad C := \frac{\partial\hat y^c}{\partial(1-\tau)}
\end{equation}
is the \emph{compensated} response.  Substituting into~\eqref{eq:fixedpt-grouped}:
\begin{equation}
\label{eq:fixedpt-slutsky}
  \delta y = -C\,\delta T'(y) - B\,\delta T(y) - C\,T''\,\delta y.
\end{equation}

\medskip
\noindent\textbf{Step 4 (Solving the fixed-point equation).}
Rearranging~\eqref{eq:fixedpt-slutsky}:
\[
  \delta y\,(1 + C\,T'') = -C\,\delta T'(y) - B\,\delta T(y).
\]
Recall the definitions of the linearized elasticities:
\[
  C = \frac{\partial\hat y^c}{\partial(1-\tau)} = \frac{y\,\zeta^c_{\mathrm{lin}}}{1-T'},
  \qquad
  B = \frac{\partial\hat y}{\partial I} = -\frac{\eta_{\mathrm{lin}}}{1-T'},
\]
where the second equality uses $\eta_{\mathrm{lin}} := -(1-T')\,\partial\hat y/\partial I > 0$ (our sign convention: $\eta_{\mathrm{lin}} > 0$ under normality of leisure).  The coefficient $1 + C\,T''$ is:
\[
  1 + C\,T''
  = 1 + \frac{y\,\zeta^c_{\mathrm{lin}}}{1-T'}\,T''
  = D(y),
\]
the deformation factor~\eqref{eq:D-def}.  Therefore:
\[
  \delta y
  = \frac{1}{D(y)}
  \left[
    -\frac{y\,\zeta^c_{\mathrm{lin}}}{1-T'}\,\delta T'(y)
    \;+\;
    \frac{\eta_{\mathrm{lin}}}{1-T'}\,\delta T(y)
  \right],
\]
which is~\eqref{eq:GWT-delta-y-lin}.  Using $\zeta^c = \zeta^c_{\mathrm{lin}}/D$ and $\eta = \eta_{\mathrm{lin}}/D$ yields~\eqref{eq:GWT-delta-y}.
\end{proof}

\begin{lemma}[Slutsky equation on the tangent hyperplane]
\label{lem:slutsky-tangent}
For the linearized Marshallian demand $\hat y(1-\tau,\,I,\,z)$ defined in~\eqref{eq:linearized-problem-pert}, the following identity holds:
\begin{equation}
\label{eq:slutsky-identity}
  \frac{\partial\hat y}{\partial(1-\tau)}
  \;=\;
  \frac{\partial\hat y^c}{\partial(1-\tau)}
  \;+\;
  y\,\frac{\partial\hat y}{\partial I}\,,
\end{equation}
where $\hat y^c$ is the Hicksian (utility-constant) demand evaluated at the current utility level~$\bar U = U(y - T(y),\,y/z)$.
\end{lemma}

\begin{proof}
The budget constraint under the linearized schedule is $c = (1-\tau)\,y' + I$.  An increase in $(1-\tau)$ by $\Delta$ raises the market value of the current bundle $(c,y) = ((1-\tau)y + I,\,y)$ by $y\,\Delta$.  By the standard Slutsky decomposition (Mas-Colell, Whinston, and Green 1995, Proposition 3.G.3), the uncompensated response equals the compensated response plus an income effect proportional to the ``endowment'' of the good whose price changed:
\[
  \frac{\partial\hat y}{\partial(1-\tau)}
  = \frac{\partial\hat y^c}{\partial(1-\tau)}
  + y\,\frac{\partial\hat y}{\partial I}.
\]
Here the ``endowment'' of $y$ is $y$ itself: at price $(1-\tau)$, the agent's implicit endowment is his current earnings level, and a change in $(1-\tau)$ makes him implicitly richer by $y$ per unit of $\Delta$.  In the classical formulation, this is the income effect from the change in the value of the ``endowment at the compensated bundle,'' which at the point of expansion equals $y$.
\end{proof}

\begin{remark}[Interpretation of the two terms in~\eqref{eq:GWT-delta-y}]
\label{rem:GWT-two-terms}
The decomposition~\eqref{eq:GWT-delta-y} has a clean economic interpretation:
\begin{enumerate}[label=(\roman*)]
  \item \textbf{Compensated effect.} The perturbation $\delta T$ changes the marginal tax rate at income~$y$ by $\delta T'(y)$.  The compensated (utility-constant) earnings response is $-\zeta^c(y)\,y/(1-T')\,\delta T'(y)$.  This term is zero if $\delta T'(y) = 0$, i.e., the perturbation changes only the tax level at~$y$ without changing the local marginal rate.  This is the \emph{distortionary} component of the behavioral response.

  \item \textbf{Income effect.} The perturbation changes the tax liability at income~$y$ by $\delta T(y)$, which is equivalent to reducing virtual income by~$\delta T(y)$.  Under normality of leisure ($\eta > 0$), the agent compensates by working more, generating an earnings increase of $\eta(y)/(1-T')\,\delta T(y) > 0$ when $\delta T(y) > 0$.  This term is zero if $\delta T(y) = 0$, i.e., the perturbation changes only marginal rates without changing the tax level at~$y$.  This is the \emph{non-distortionary} component: the agent voluntarily adjusts labor supply in response to being poorer or richer.
\end{enumerate}

\noindent
Both effects incorporate the deformation correction through the structural elasticities $\zeta^c = \zeta^c_{\mathrm{lin}}/D$ and $\eta = \eta_{\mathrm{lin}}/D$.  On a linear baseline ($T'' = 0$, $D = 1$), the structural and linearized elasticities coincide.
\end{remark}

\begin{remark}[Consistency check: pure marginal-rate perturbation]
\label{rem:GWT-consistency}
Setting $\delta T(y) = \max\{y - y^*,\,0\}$ for fixed $y^*$, we have $\delta T'(y) = 1$ and $\delta T(y) = y - y^*$ for $y > y^*$.  The compensated effect at $y > y^*$ is $-\zeta^c\,y/(1-T')$, recovering~\eqref{eq:dy-deformation} (a pure compensated response to $d\tau = 1$).  The income effect contributes $+\eta\,(y-y^*)/(1-T')$, reflecting the lump-sum tax increase of $(y-y^*)$ on agents above~$y^*$.  These are exactly the two effects that will appear in the Saez perturbation of Section~\ref{sec:saez-specific}.
\end{remark}

\begin{remark}[Relationship to Golosov, Tsyvinski, and Werquin (2014)]
\label{rem:GWT-relation}
Proposition~\ref{prop:GWT-1D} is the specialization of GWT's Proposition~1 (their equation~(8)) to the case $S = 1$ with a single income dimension.  In their general notation, the $2S \times 2S$ identity~$\mathbf{i}_{2S}$, the compensated elasticity matrix~$\mathbf{E}^c_p$, the income-effect vector~$\mathbf{I}_p$, and the Hessian~$D^2 T_s$ all reduce to scalars:
\[
  \mathbf{E}^c \to C = \frac{y\,\zeta^c_{\mathrm{lin}}}{1-T'},
  \qquad
  \mathbf{I} \to B = -\frac{\eta_{\mathrm{lin}}}{1-T'},
  \qquad
  D^2 T \to T''(y),
\]
and their matrix inverse $[\mathbf{i}_{2S} - \sum_s \mathbf{E}^c_s D^2 T_s]^{-1}$ collapses to the scalar $1/D(y)$.  The fixed-point equation~\eqref{eq:fixedpt-raw} is the one-dimensional version of their equation~(10), and the solution~\eqref{eq:GWT-delta-y-lin} is the one-dimensional version of~(8).
\end{remark}

\subsection{The Lagrangian and a general perturbation}
\label{sec:perturb-setup}

We begin by deriving the effect on the planner's objective of an \emph{arbitrary} perturbation $\delta T(y)$ of the tax schedule. This yields general expressions for the mechanical and behavioral effects in terms of $\delta T$. We then specialize to the Saez perturbation (a marginal rate increase in a narrow band) to obtain the pointwise tax formula.

\paragraph{The Lagrangian.}
Recall from Section~\ref{sec:mechanism}:
\[
\mathcal{L}(T)
=
...
\paragraph{A general perturbation.}
Consider a perturbed schedule $\tilde T(y) = T(y) + \epsilon\,\delta T(y)$ for an arbitrary admissible direction $\delta T: \R_+ \to \R$, with $\epsilon > 0$ a perturbation parameter. Under the perturbed schedule, each agent's earnings adjust from $y(z)$ to $y(z;\tilde T)$. The Gâteaux derivative of $\mathcal{L}$ at $T$ in the direction $\delta T$ is
\begin{equation}
...
components. At the optimum, $D\mathcal{L}(T;\, \delta T) = 0$ for all admissible directions $\delta T$---this is the necessary condition for optimality in function space.
...
= -U_c\,\delta T(y).
\]
Thus the only welfare change comes from the direct tax change
...
who was at an optimum. Defining the \textbf{marginal social welfare weight}
\begin{equation}
\label{eq:g-pert}
...
\end{equation}
This is the \textbf{mechanical effect per agent}: the net gain (revenue
minus welfare cost) from the direct tax change, with behavior
held fixed.
...
\paragraph{The income-effect revenue.}
Recall the \textbf{structural income effect on earnings}:
\[
  \eta(z) = \frac{\eta_{\mathrm{lin}}(z)}{D(y(z))},
  \qquad
  \eta_{\mathrm{lin}}(z) := -(1-T')\,\frac{\partial \hat y}{\partial I} > 0,
\]
where $\hat y$ is earnings on the linearized schedule and $D$ is the deformation factor~\eqref{eq:D-def}. Three ingredients enter the linearized definition:
\begin{itemize}[leftmargin=2em]
  \item $\partial \hat y/\partial I < 0$: the raw income effect---how earnings change when nonlabor income rises by \$1. Under normality of leisure, richer agents work less.
  \item The minus sign: a sign convention so that $\eta_{\mathrm{lin}} > 0$ under normality, measuring how much earnings \emph{fall} per unit increase in nonlabor income.
  \item The $(1-T')$ factor: normalizes the income effect per dollar of \emph{net-of-tax} income rather than gross income. This ensures the Slutsky relation in earnings space takes the clean form $\zeta^c_{\mathrm{lin}} = \zeta^u_{\mathrm{lin}} + \eta_{\mathrm{lin}}$.
\end{itemize}
The structural income effect $\eta$ incorporates the deformation correction: when the agent adjusts income in response to a lump-sum change, he again slides along the curved schedule, with the same feedback mechanism as in the compensated case.

The perturbation changes the tax paid by $\delta T(y)$, which is equivalent to reducing nonlabor income by $\delta T(y)$.  By Proposition~\ref{prop:GWT-1D}, the income-effect component of the behavioral response is determined by the level $\delta T(y)$ of the perturbation.  Since the change in virtual income is $\Delta I = -\delta T(y)$ (the agent is poorer by $\delta T(y)$), the income-effect behavioral response in earnings is
\[
  dy_{\text{inc}}
  = \frac{\partial y}{\partial I}\cdot(-\delta T(y))
  = \frac{\eta}{1-T'}\cdot\delta T(y),
\]
where $\eta = \eta(y) > 0$ is the \emph{structural} income effect (inclusive of the deformation feedback).  The sign is positive: under normality of leisure, the poorer agent works \emph{more}.  The revenue consequence is $T'(y)\cdot dy_{\text{inc}}$, giving
\[
  \text{income-effect revenue per agent}
  = +\mu\,\frac{T'}{1-T'}\,\eta\cdot\delta T(y) \;>\; 0.
\]
That is, the income effect generates \emph{additional} revenue beyond the mechanical effect.

\paragraph{Absorbing into term (a).}
Combine the original mechanical effect (a) $= \mu\,\delta T(y)(1-g(z))$ with the income-effect revenue:
\begin{align*}
...
\end{equation}
The adjusted weight $\hat g(z)$ is the net social cost of extracting a dollar from type $z$, \emph{after crediting back} the income-effect revenue bonus.  A lump-sum tax increase \emph{raises} earnings via the income effect when leisure is normal ($\eta > 0$), so the income-effect channel generates additional revenue, making extraction cheaper than $g$ alone suggests: $\hat g(z) < g(z)$, which is why the income-effect correction enters with a minus sign.  Under quasilinear preferences ($\eta = 0$), $\hat g = g$.

\paragraph{The general $dM$ and $dB$ for arbitrary $\delta T$.}
Aggregating over all types, the total effect of an arbitrary perturbation $\delta T$ decomposes as:
\begin{align}
  dM &= \int_{\zlow}^{\zbar} \mu\,\delta T(y(z))\,\bigl(1 - \hat g(z)\bigr)\,f(z)\,dz,
  \label{eq:dM-general} \\
  dB &= \int_{\zlow}^{\zbar} \mu\,T'(y(z))\,\frac{\partial y^c}{\partial\epsilon}\,f(z)\,dz,
  \label{eq:dB-general}
\end{align}
where $dM$ is the mechanical effect (direct revenue net of welfare cost, including the income-effect revenue bonus) and $dB$ is the compensated behavioral effect.

To express $dB$ in terms of $\delta T$ and the structural compensated elasticity, note that the perturbation $\delta T$ changes the marginal tax rate at income $y$ by $\delta T'(y)$. The \emph{total} compensated earnings response---inclusive of the feedback from the curvature of the baseline schedule---is
\[
  \frac{\partial y^c}{\partial\epsilon} = -\zeta^c(y)\,\frac{y}{1-T'(y)}\,\delta T'(y),
\]
where $\zeta^c(y)$ is the \emph{structural} compensated ETI defined in~\eqref{eq:ETI-def}, which already incorporates the deformation factor $D(y)$. Substituting and changing variables from $z$ to $y$:
\begin{equation}
\label{eq:dB-general-explicit}
  dB = -\int_0^{\infty} \mu\,\frac{T'(y)}{1-T'(y)}\,\zeta^c(y)\,y\,\delta T'(y)\,h(y)\,dy.
\end{equation}
The behavioral effect depends on $\delta T'(y)$---the change in the \emph{marginal} rate---not on $\delta T(y)$ (the change in the tax level). Only agents at incomes where the marginal rate changes adjust their compensated labor supply; agents who face only a lump-sum shift (unchanged marginal rate, $\delta T'(y) = 0$) have no compensated response.

At the optimum, $dM + dB = 0$ for every admissible $\delta T$.

\begin{remark}[Budget-balance normalization]
\label{rem:budget_balance}
The adjusted welfare weights integrate to one: $\int_0^\infty \hat g(y)\,h(y)\,dy = 1$. To see this, consider the uniform lump-sum perturbation $\delta T(y) = 1$ for all $y$ (a uniform tax increase of one dollar on every earner). This does not change any marginal rate, so $\partial y^c/\partial\epsilon = 0$ and $dB = 0$. At the optimum $dM + dB = 0$ requires $dM = 0$, i.e., $\int (1 - \hat g)\,h\,dy = 0$.
\end{remark}

%--------------------------------------------------------------------------
\subsection{The Saez perturbation}
\label{sec:saez-specific}
%--------------------------------------------------------------------------

We now specialize to the particular perturbation that yields a pointwise formula for the optimal marginal tax rate.
...
$y^*+\varepsilon$ by exactly $\varepsilon\,d\tau$ (a lump-sum increase) and increases
the marginal rate faced by earners in the band by $d\tau$. The reform
...
\subsection{Evaluating $dM$ for the Saez perturbation}
\label{sec:mech}

We now evaluate the general mechanical effect \eqref{eq:dM-general} for the specific Saez perturbation $\delta T(y)$ defined above. Substituting $\delta T(y)$ and changing variables from $z$ to $y$ using $f(z)\,dz = h(y)\,dy$, the integral has three pieces:
\begin{enumerate}[label=(\roman*)]
...
\subsection{Evaluating $dB$ for the Saez perturbation}
\label{sec:behav}

The Saez perturbation changes the marginal rate by $\delta T'(y) = d\tau$ inside the band $[y^*, y^*+\varepsilon]$ and $\delta T'(y) = 0$ elsewhere. By \eqref{eq:dB-general-explicit}, only agents inside the band contribute to $dB$:
\[
  dB = -\int_{y^*}^{y^*+\varepsilon} \mu\,\frac{T'(y)}{1-T'(y)}\,\zeta^c(y)\,y\,d\tau\,h(y)\,dy.
\]
Here $\zeta^c(y)$ is the structural compensated ETI~\eqref{eq:ETI-def}, which already accounts for the feedback from the curvature of the baseline schedule through the deformation factor~$D(y)$.
Within the band, the integrand is continuous, so evaluating at $y^*$:
\[
  dB = -\varepsilon\,d\tau\,\mu\,\frac{T'(y^*)}{1-T'(y^*)}\,\zeta^c(y^*)\,y^*\,h(y^*) + O(\varepsilon^2\,d\tau).
\]
Dividing by $\mu$:
\begin{equation}
\label{eq:dB-pert}
...
The result confirms the general principle from \eqref{eq:dB-general-explicit}: agents outside the band face no change in the marginal rate ($\delta T' = 0$), so their compensated response is zero. Their only effect is through the mechanical channel (the lump-sum increase $\varepsilon\,d\tau$), which is captured in $dM$ via $\hat g$.
...
\begin{itemize}
  \item $1/\zeta^c(y^*)$: the inverse structural compensated ETI
...
\begin{remark}[Unpacking the structural elasticity]
\label{rem:unpack-structural}
Substituting $\zeta^c(y) = \zeta^c_{\mathrm{lin}}(y)/D(y)$ from~\eqref{eq:ETI-def} into~\eqref{eq:FOC-pert} and using $D(y^*) = 1 + \frac{y^*\,\zeta^c_{\mathrm{lin}}(y^*)}{1-T'(y^*)}T''(y^*)$, the formula can equivalently be written as
\[
  \frac{T'(y^*)}{1-T'(y^*) + y^*\,\zeta^c_{\mathrm{lin}}(y^*)\,T''(y^*)}
  = \frac{1}{\zeta^c_{\mathrm{lin}}(y^*)}
  \cdot \frac{1}{y^*\,h(y^*)}
  \int_{y^*}^{\infty}
  \bigl(1-\hat g(y)\bigr)\,h(y)\,dy,
\]
which is the form appearing in Golosov, Tsyvinski, and Werquin (2014, equation~28). This makes the role of $T''$ explicit: on a convex schedule ($T''>0$), the denominator on the left is larger, so the effective tax wedge is lower for any given level of the right-hand side---reflecting the stabilizing feedback discussed in Section~\ref{sec:ETI}. When characterizing the optimum (where $T$ is the unknown), the structural elasticity $\zeta^c$ is more natural because the formula does not involve $T''$. When evaluating a reform of a \emph{given} schedule, the linearized elasticity $\zeta^c_{\mathrm{lin}}$ is what one estimates from the data, and the $T''$ correction must be applied explicitly.
\end{remark}

\begin{remark}[Equivalence of the Hamiltonian and perturbation formulas]
  \label{rem:equivalence}
  
  The Hamiltonian formula \eqref{eq:general_tax}, derived in type-space, and the Saez formula \eqref{eq:FOC-pert}, derived in income-space, are the same equation expressed in different coordinates. The deformation factor $D(y)$ from~\eqref{eq:D-def} does not appear in the Hamiltonian because it is absorbed into the Jacobian $y'(z)$ that converts the type-density $zf(z)$ into the income-density $yh(y)$. We now show this explicitly.
  
  \paragraph{Step 1: The two formulas to reconcile.}
  Ignoring the redistribution integrals (which encode the same information packaged differently), the Hamiltonian formula has the structure
  \begin{equation}
  \label{eq:ham-structure}
    \frac{T'}{1-T'} \;\propto\; \frac{1+\varepsilon^u}{\varepsilon^c} \cdot \frac{1}{z\,f(z)},
  \end{equation}
  while the Saez formula has
  \begin{equation}
  \label{eq:saez-structure}
    \frac{T'}{1-T'} \;\propto\; \frac{1}{\zeta^c} \cdot \frac{1}{y\,h(y)}
    = \frac{D}{\zeta^c_{\mathrm{lin}}} \cdot \frac{1}{y\,h(y)}
    = \frac{D}{\varepsilon^c} \cdot \frac{1}{y\,h(y)},
  \end{equation}
  where we used $\zeta^c = \zeta^c_{\mathrm{lin}}/D$ from~\eqref{eq:ETI-def} and $\zeta^c_{\mathrm{lin}} = \varepsilon^c$ (the linearized ETI coincides with the compensated labor supply elasticity since $y = zn$ with $z$ fixed for each agent). The density transformation $h(y) = f(z)/y'(z)$ from~\eqref{eq:H-def} gives $y\,h(y) = y\,f(z)/y'(z)$. Equating~\eqref{eq:ham-structure} and~\eqref{eq:saez-structure} and cancelling $\varepsilon^c$, equivalence reduces to:
  \begin{equation}
  \label{eq:Jacobian-D}
    \boxed{y'(z) = \frac{y(z)}{z} \cdot \frac{1+\varepsilon^u(z)}{D(y(z))}.}
  \end{equation}
  
  \paragraph{Step 2: Deriving \eqref{eq:Jacobian-D} from the agent's FOC.}
  The household first-order condition is
  \begin{equation}
  \label{eq:agent-foc-equiv}
    -\frac{U_n(c(z),\, y(z)/z)}{U_c(c(z),\, y(z)/z)} = z\bigl(1-T'(y(z))\bigr),
  \end{equation}
  where $c(z) = y(z) - T(y(z))$. Differentiate both sides with respect to $z$, using $n = y/z$ and $c = y - T(y)$. The right-hand side gives
  \[
    \frac{d}{dz}\bigl[z(1-T'(y))\bigr] = (1-T') - z\,T''(y)\,y'(z).
  \]
  For the left-hand side, define $\Phi(c,n) := -U_n(c,n)/U_c(c,n)$ and compute
  \[
    \frac{d\Phi}{dz} = \Phi_c\,c'(z) + \Phi_n\,n'(z),
  \]
  where $c'(z) = (1-T'(y))\,y'(z)$ and $n'(z) = (y'(z) - n)/z$. Setting the two sides equal and solving for $y'(z)$:
  \begin{equation}
  \label{eq:yprime-general}
    y'(z) = \frac{(1-T') - \Phi_n\,(-n/z)}{-\Phi_c(1-T') - \Phi_n/z + z\,T''}.
  \end{equation}
  We now identify the terms. From the definitions of the elasticities in Section~\ref{sec:elasticities}, after computing $\Phi_c$ and $\Phi_n$ and using the household FOC $\Phi = w = z(1-T')$:
  \begin{align*}
    \Phi_n &= \frac{U_n^2 U_{cc}/U_c^2 - 2U_n U_{cn}/U_c + U_{nn}}{U_c}
    = -\frac{U_n/n}{\varepsilon^c \, U_c}, \\
    \Phi_c &= \frac{U_n U_{cc}/U_c - U_{cn}}{U_c}.
  \end{align*}
  Substituting and simplifying (the details mirror the verification of~\eqref{eq:key_ratio}), the numerator of~\eqref{eq:yprime-general} becomes
  \[
    \text{Num} = (1-T')\left(1 + \frac{n\,\Phi_n}{w}\right) = \frac{(1-T')(1+\varepsilon^u)}{\varepsilon^c} \cdot \varepsilon^c = (1-T')\cdot\frac{1+\varepsilon^u}{\varepsilon^c}\cdot\varepsilon^c,
  \]
  which simplifies to $(1-T')(1+\varepsilon^u)$, and the denominator becomes
  \[
    \text{Den} = \frac{z}{\varepsilon^c}\cdot\frac{1-T'}{y}\cdot D(y),
  \]
  where $D(y) = 1 + \frac{y\,\varepsilon^c}{1-T'}\,T''(y) = 1 + \frac{y\,\zeta^c_{\mathrm{lin}}}{1-T'}\,T''(y)$ is the deformation factor.
  
  Dividing:
  \[
    y'(z) = \frac{(1-T')(1+\varepsilon^u)}{\frac{z}{\varepsilon^c}\cdot\frac{1-T'}{y}\cdot D}
    = \frac{y\,\varepsilon^c\,(1+\varepsilon^u)}{z\,D}
    = \frac{y}{z}\cdot\frac{1+\varepsilon^u}{D},
  \]
  which is~\eqref{eq:Jacobian-D}. (In the last step we used $\varepsilon^c \cdot (1+\varepsilon^u)/\varepsilon^c = 1+\varepsilon^u$.)
  
  \paragraph{Step 3: Interpretation.}
  Identity~\eqref{eq:Jacobian-D} shows that the schedule curvature $T''(y)$, which enters the Saez formula through $D(y)$, enters the Hamiltonian formula through the Jacobian $y'(z)$. The Hamiltonian optimizes directly over allocations $y(z)$ and derives the tax schedule afterward---$T''$ is a \emph{consequence} of the optimal $y(z)$, not an input. The perturbation approach takes $T$ as given and perturbs it, so agents must respond on the curved schedule---$D$ is the \emph{price} of working in income-space, encoding the feedback between the agent's response and the slope of the existing schedule.
  
  The Jacobian $y'(z)$ captures two distinct effects: (i)~the behavioral response of agents to the local incentives (the elasticity), and (ii)~the amplification or dampening from schedule curvature (the deformation). In type-space these are bundled together inside $y'(z)$; in income-space they are separated into $\varepsilon^c$ (or $\zeta^c_{\mathrm{lin}}$) and $D$. This is precisely why Golosov, Tsyvinski, and Werquin~(2014, p.~21) observe that their optimality PDE in type-space ``does not feature the deformation matrix $D(x)$, so that we can solve directly for the marginal tax rates.''
  
  \paragraph{Quasilinear check.}
  With $U(c,n) = c - v(n)$, we have $\varepsilon^c = \varepsilon^u = 1/\gamma$ where $\gamma = nv''/v'$. The FOC is $v'(n) = z(1-T')$. Differentiating:
  \[
    v''(n)\,\frac{y'z - y}{z^2} = (1-T') - zT''y',
  \]
  so
  \[
    y'(z) = \frac{(1-T') + nv''/z}{v''/z + zT''} = \frac{(1-T')(1+\gamma)}{1 + \frac{y\,(1/\gamma)}{1-T'}\,T''} = \frac{y}{z}\cdot\frac{1+\varepsilon^u}{D(y)},
  \]
  confirming~\eqref{eq:Jacobian-D} directly.
  \end{remark}
...
\paragraph{Numerical quadrature for the resource constraint.}
In addition to integrating the ODE system, the algorithm must evaluate the resource constraint $\int_{\zlow}^{\zbar}[y(z) - c(z)]\,f(z)\,dz = G$. This integral is computed alongside the ODE using the \emph{trapezoidal rule}: at each step from $z_i$ to $z_{i+1}$ (with step size $\Delta z = z_{i+1} - z_i$), the contribution to the integral is approximated as
\[
  \int_{z_i}^{z_{i+1}} [y(z) - c(z)]\,f(z)\,dz \approx \frac{\Delta z}{2}\bigl\{[y^i - c^i]\,f(z_i) + [y^{i+1} - c^{i+1}]\,f(z_{i+1})\bigr\},
\]
which is the average of the integrand at the two endpoints times the step width. Since the grid is log-spaced ($z_{i+1}/z_i = \kappa$), the step size $\Delta z = z_i(\kappa - 1)$ is small at the bottom and large at the top. A running sum $R$ accumulates these contributions as the ODE is integrated forward; the residual $r_2 = R - G$ then measures how far off the resource constraint is.
...
For a Pareto density, $d\log f/d\log z = -(\alpha+1)$, a constant. More generally, for any density with a regularly varying tail, $d\log f/d\log z$ is bounded or grows at most logarithmically. This is dominated by the first term, which diverges as a power $z^{(1+\gamma)(1+\nu)/\gamma}$ with a strictly positive exponent.
...
\begin{proof}
By Proposition~\ref{prop:nature_br}, the unique minimizer is $m^* = e^{-g_\alpha/\theta}/Z(\alpha)$ where $Z(\alpha) = \int e^{-g_\alpha/\theta}f\,dz$. Substituting into $\calL$ and using $\log m^* = -g_\alpha/\theta - \log Z$:
\begin{align*}
  V(\alpha) = \calL(a_\alpha, m^*)
  &= \int g_\alpha\,\frac{e^{-g_\alpha/\theta}}{Z}\,f
  + \theta \int \frac{e^{-g_\alpha/\theta}}{Z}
    \bigl(-g_\alpha/\theta - \log Z\bigr)\,f \\
  &= \frac{1}{Z}\int g_\alpha\,e^{-g_\alpha/\theta}f
  - \frac{1}{Z}\int g_\alpha\,e^{-g_\alpha/\theta}f
  - \theta\log Z
  = -\theta\log Z(\alpha).
\end{align*}
This is the \textbf{entropic dual}: the government's value function reduces to $V(\alpha) = -\theta\log Z(\alpha)$.
...
lies in a bounded interval.  We show that $W(u_0, q) \to -\infty$ as $|u_0| \to \infty$, so extreme values of $u_0$ cannot be optimal.
...
\end{equation}
The entropic dual $W(a) = -\theta\log Z$ was verified in the proof of Proposition~\ref{prop:envelope}.
...
$\psi(z)$ is not constant on $\calZ$, with $\int_{\calZ} \psi(z)\,f(z)\,dz = 1$.
\end{assumption}

Since $\mu = 1$ under quasilinear preferences (Section~\ref{sec:QL_example}) and $\psi$ is non-constant with $\int \psi\,f = 1$, we have $\operatorname*{ess\,inf}\psi < 1 < \operatorname*{ess\,sup}\psi$: there exist types with $\psi(z) > \mu$ (society values their utility more than the fiscal cost) and types with $\psi(z) < \mu$ (the reverse). This is the generic case for any redistributive planner.

\begin{lemma}[Boundedness of optimal $u_0$]
...
\begin{remark}[How Lemma~\ref{lem:coercivity} restricts the domain]
\label{rem:coercivity_use}
Lemma~\ref{lem:coercivity} implies that extreme values of $u_0$
...
By Lemma~\ref{lem:coercivity}, there exists $M < \infty$ such that
\[
...
Under Assumptions~\ref{as:bounded}--\ref{as:mu_interior},
...
non-constant (Assumption~\ref{as:mu_interior}) and
$u_{0,1} \neq u_{0,2}$.
...
\label{cor:T_unique}
Under quasilinear isoelastic preferences
and Assumptions~\ref{as:bounded}--\ref{as:mu_interior}:
\begin{enumerate}[label=(\roman*)]
...
%--------------------------------------------------------------------------
\subsection{Extension: the quasilinear unbounded-support case}
\label{sec:unbounded_QL}
%--------------------------------------------------------------------------

The preceding subsection establishes existence and
uniqueness of the optimal allocation under
Assumption~\ref{as:bounded} (bounded support:
$\zbar < \infty$).  This subsection removes that
restriction.  We prove existence and uniqueness of
$(u_0^*, q^*)$ for the fixed-$\mu$ Lagrangian, and
validity of the robust Saez formula, when
$\calZ = [\zlow, \infty)$.

The bounded-support assumption was used at three points:
\begin{enumerate}[label=(\roman*)]
\item the uniform output bound
  $q(z) \leq \bar q = \zbar^{(1+\gamma)^2/\gamma}$
  (Step~3, Definition~\ref{def:A_q});
\item Helly's selection theorem on the compact interval
  $[\zlow, \zbar]$ (Lemma~\ref{lem:Aq_compact});
\item the global bound $|r_q(z)| \leq C_r$
  (Lemma~\ref{lem:Cr_bound}), which enters both the
  coercivity proof (Lemma~\ref{lem:coercivity}) and the
  dominated convergence argument for continuity of~$W$
  (Proposition~\ref{prop:existence}).
\end{enumerate}
All three uses are replaced below by \emph{localization}
arguments: on every compact sub-interval $[\zlow, K]$,
admissibility still provides uniform bounds, and Helly still
applies.  The remaining challenge---dominating the integrand
$e^{-g_a/\theta}\,f$ on the full unbounded
domain---is handled by a one-sided lower bound on $g_a$
that requires only $\psi \geq 0$ and $|u_0| \leq M$, not a
global bound on $|r_q|$.

The perturbation formula is a logically separate issue: it
requires tail integrability hypotheses that go beyond what
existence and uniqueness need.  We isolate these in
Assumption~\ref{as:tail_formula_unbounded} and prove the
robust Saez formula on unbounded support in
Proposition~\ref{prop:robust_saez_unbounded}.  A capstone
theorem (Theorem~\ref{thm:unbounded_QL_extension})
collects the full result.

Throughout this subsection we keep the quasilinear isoelastic
preferences of \S\ref{sec:quasilinear}, the fixed multiplier
$\mu > 0$, and the admissibility requirement
Assumption~\ref{as:admissible}.


%..........................................................................
\subsubsection{Assumptions}

We replace Assumption~\ref{as:bounded} with:

\begin{assumption}[Unbounded support]
\label{as:unbounded_QL}
$\calZ = [\zlow, \infty)$ with $\zlow > 0$.
\end{assumption}

\begin{assumption}[Bounded welfare weights]
\label{as:psi_bounded_unbounded}
$\psi \geq 0$ and $\psi \in L^\infty(\calZ)$.
\end{assumption}

\begin{remark}[When bounded $\psi$ is automatic]
\label{rem:psi_bounded_automatic}
Assumption~\ref{as:psi_bounded_unbounded} is automatically
satisfied when $\calZ$ is bounded (any measurable function
on a bounded interval with a finite essential supremum is in
$L^\infty$).  On unbounded support it rules out welfare
weights that grow without bound, which would be economically
unusual.  Every standard specification---utilitarian
($\psi \equiv 1$), Rawlsian ($\psi$ concentrated at
$\zlow$), generalized utilitarian with decreasing $\psi$,
and any $\psi$ with bounded range---satisfies
Assumption~\ref{as:psi_bounded_unbounded}.
\end{remark}

\begin{assumption}[Interior welfare weights]
\label{as:mu_interior_unbounded}
There exist $\delta_-, \delta_+ > 0$ and measurable sets
$A_-, A_+ \subset \calZ$ with $\int_{A_-} f > 0$ and
$\int_{A_+} f > 0$ such that
\[
  \psi(z) \leq \mu - \delta_- \quad \text{on } A_-,
  \qquad
  \psi(z) \geq \mu + \delta_+ \quad \text{on } A_+.
\]
\end{assumption}

\begin{remark}[Relation to Assumption~\ref{as:mu_interior}]
\label{rem:mu_interior_unbounded}
Assumption~\ref{as:mu_interior_unbounded} is the exact condition used
in the coercivity argument.  Under the normalization of
\S\ref{sec:existence}, namely $\mu = 1$ and
$\int_{\calZ} \psi(z) f(z)\,dz = 1$, it follows from the
non-constancy of $\psi$: if $\psi$ were not both above and below
$\mu$ on sets of positive $f$-measure, then either $\psi \geq \mu$
a.e.\ or $\psi \leq \mu$ a.e., and the normalization would force
$\psi = \mu$ a.e.
\end{remark}

\begin{remark}[No tail condition]
\label{rem:no_tail_unbounded}
We impose no condition of the form
``$\psi(z) \leq \mu$ for large~$z$'' or any moment/tail
condition on~$f$ for the fixed-$\mu$ existence/uniqueness
result.  Such conditions would be satisfied by standard
calibrations, but they are not needed: every argument that
previously required a global bound on $r_q(z)$ can be
localized to bounded subsets of $\calZ$, where admissibility
provides the required uniform bounds automatically.
\end{remark}


%..........................................................................
\subsubsection{The feasible set on $[\zlow,\infty)$}

Admissibility still implies a pointwise growth bound.  Since
$0 \leq T'(y) \leq 1$, the household FOC gives
$n(z)^\gamma = z(1-T'(y(z))) \leq z$, hence
\[
  y(z) = z n(z) \leq z^{(1+\gamma)/\gamma},
  \qquad
  q(z) = y(z)^{1+\gamma} \leq \bar q(z)
  \equiv z^{(1+\gamma)^2/\gamma}.
\]
The bound is now \emph{$z$-dependent}, replacing the uniform
$\bar q = \zbar^{(1+\gamma)^2/\gamma}$ of
Definition~\ref{def:A_q}.

\begin{definition}[Unbounded-support output set]
\label{def:A_q_unbounded}
Define
\[
  \mathcal{Q}_\infty
  = \Bigl\{q : [\zlow,\infty) \to \R_+ \,\Big|\,
  q \text{ is nondecreasing and }
  q(z) \leq \bar q(z) \text{ for all } z \geq \zlow\Bigr\}.
\]
For $M > 0$, set
\[
  \mathcal{A}_{q,\infty}(M) = [-M,M] \times \mathcal{Q}_\infty,
  \qquad
  \mathcal{A}_{q,\infty} = \R \times \mathcal{Q}_\infty.
\]
\end{definition}

\begin{lemma}[Equivalence of parameterizations on unbounded support]
\label{lem:Aq_equiv_unbounded}
Under Assumptions~\ref{as:unbounded_QL},
\ref{as:admissible}, and the quasilinear isoelastic preferences of
\S\ref{sec:quasilinear}:
\begin{enumerate}[label=(\roman*)]
\item The map $(u_0,q) \mapsto (c,y)$ defined by
  \eqref{eq:q_to_y}--\eqref{eq:q_consumption} is a bijection from
  $\mathcal{A}_{q,\infty}$ onto the set of admissible IC allocations
  on $[\zlow,\infty)$.
\item Under this bijection, $W(u_0,q) = W(c,y)$.
\end{enumerate}
\end{lemma}
...
\begin{proof}
The proof parallels Lemma~\ref{lem:Aq_equiv}, with the
uniform bound $q \leq \bar q$ replaced by the pointwise
bound $q(z) \leq \bar q(z)$.

\emph{Forward: $(u_0, q) \in \mathcal{A}_{q,\infty}
\Rightarrow$ admissible IC allocation.}
Given $(u_0,q)$ with $q$ nondecreasing and
$q(z) \leq \bar q(z)$:
$y = q^{1/(1+\gamma)}$ is nondecreasing, $\calU$ satisfies the
envelope condition by \eqref{eq:q_envelope}, and IC follows from
Lemma~\ref{lem:FOA}.  At every regular point of the on-path tax,
Proposition~\ref{prop:marginal} gives
\[
  T'(y(z)) = 1 - \frac{(y(z)/z)^\gamma}{z}.
\]
Because $q(z) \leq \bar q(z)$, we have
$y(z) \leq z^{(1+\gamma)/\gamma}$ and therefore
$(y(z)/z)^\gamma \leq z$, so $T'(y(z)) \geq 0$; the inequality
$T'(y(z)) \leq 1$ is immediate.

\emph{Reverse: admissible IC allocation $\Rightarrow
(u_0, q) \in \mathcal{A}_{q,\infty}$.}
If an admissible tax schedule implements an IC allocation
$(c,y)$, set $u_0 = \calU(\zlow)$, $q = y^{1+\gamma}$.
Monotonicity of $y$ gives $q$ nondecreasing.  The bound
$0 \leq T' \leq 1$ implies
$q(z) \leq \bar q(z)$ exactly as above.
Equations \eqref{eq:q_to_y}--\eqref{eq:q_consumption}
reproduce $(c,y)$.

\emph{Part~(ii).}
$W$ depends on the allocation only through $\calU$, $y$, and
$T(y)$, all of which are determined by
\eqref{eq:q_to_y}--\eqref{eq:q_tax}.
\end{proof}

\begin{remark}[Local uniform bounds]
\label{rem:local_bounds_unbounded}
Although $\mathcal{Q}_\infty$ is not uniformly bounded on all of
$[\zlow,\infty)$, it is uniformly bounded on every compact interval.
Indeed, if $K \geq \zlow$, then for every $q \in \mathcal{Q}_\infty$
and every $z \in [\zlow,K]$,
\[
  0 \leq q(z) \leq q(K) \leq \bar q(K).
\]
This uses only monotonicity of $q$ and of $\bar q$.
All compactness and dominated-convergence arguments below rely only on
this localized bound.
\end{remark}


%..........................................................................
\subsubsection{Compactness via diagonal extraction}

\begin{lemma}[Sequential compactness of $\mathcal{Q}_\infty$]
\label{lem:Aq_compact_unbounded}
$\mathcal{Q}_\infty$ is convex and sequentially compact in the
pointwise topology.  Consequently,
$\mathcal{A}_{q,\infty}(M)$ is sequentially compact for every
$M < \infty$.
\end{lemma}

\begin{proof}
\emph{Convexity.}
If $q_1, q_2 \in \mathcal{Q}_\infty$ and $t \in [0,1]$,
then $q_t = tq_1 + (1-t)q_2$ is nondecreasing and satisfies
$q_t(z) \leq t\bar q(z) + (1-t)\bar q(z) = \bar q(z)$.

\emph{Sequential compactness.}
Let $\{q_n\} \subset \mathcal{Q}_\infty$.
For each integer $k \geq 1$, set $K_k = \zlow + k$.  By
Remark~\ref{rem:local_bounds_unbounded}, the restrictions
of $q_n$ to $[\zlow,K_k]$ are nondecreasing and uniformly
bounded by $\bar q(K_k)$.  Helly's selection theorem
(Lemma~\ref{lem:helly}) yields a subsequence converging
pointwise on $[\zlow,K_1]$.  Iterating on
$[\zlow,K_2], [\zlow,K_3], \ldots$ and taking the diagonal
subsequence, we obtain $q_{n_j}(z) \to q(z)$ for every
$z \in [\zlow,\infty)$.  The pointwise limit of
nondecreasing functions is nondecreasing, and the bound
$0 \leq q_{n_j}(z) \leq \bar q(z)$ passes to the limit, so
$q \in \mathcal{Q}_\infty$.

\emph{Product space.}
Given $(u_{0,n}, q_n) \in \mathcal{A}_{q,\infty}(M)$:
extract a subsequence with $q_{n_k} \to q$ by the diagonal
argument, then a further subsequence with
$u_{0,n_{k_j}} \to u_0 \in [-M,M]$ by
Bolzano--Weierstrass.
\end{proof}


%..........................................................................
\subsubsection{Coercivity by localization}

We retain the decomposition
\begin{equation}
\label{eq:g_decompose_unbounded}
  g_a(z) = r_q(z) + (\psi(z)-\mu)u_0,
\end{equation}
with
\begin{equation}
\label{eq:r_q_unbounded}
  r_q(z)
  = (\psi(z)-\mu)\int_{\zlow}^z \frac{q(s)}{s^{2+\gamma}}\,ds
  + \mu\!\left[q(z)^{1/(1+\gamma)}
  - \frac{q(z)}{(1+\gamma)z^{1+\gamma}}\right].
\end{equation}
On unbounded support there is no global constant $C_r$
controlling $r_q$ for all $z$
(cf.\ Lemma~\ref{lem:Cr_bound}).  The key observation is
that coercivity only needs a bound on a \emph{bounded}
subset of types where $\psi-\mu$ has a strict sign.

\begin{lemma}[Localization of sign sets]
\label{lem:localize_sign_sets_unbounded}
Under Assumption~\ref{as:mu_interior_unbounded} and the fact that
$f$ is a probability density, there exist bounded measurable sets
$\widetilde A_- \subset A_-$ and $\widetilde A_+ \subset A_+$ and a
constant $K > \zlow$ such that
$\widetilde A_\pm \subset [\zlow,K]$ and
$\int_{\widetilde A_\pm} f > 0$.
\end{lemma}

\begin{proof}
We treat $A_-$.  Since $f$ is a probability density,
\[
  \int_{A_-} f
  = \lim_{K\to\infty} \int_{A_- \cap [\zlow,K]} f.
\]
The left-hand side is strictly positive by
Assumption~\ref{as:mu_interior_unbounded}, so for some finite $K$ we
must have $\int_{A_- \cap [\zlow,K]} f > 0$.  Set
$\widetilde A_- = A_- \cap [\zlow,K]$.  The argument for $A_+$ is
identical.  Enlarging $K$ if necessary ensures both
$\widetilde A_\pm \subset [\zlow,K]$.
\end{proof}

\begin{lemma}[Local bound on $r_q$]
\label{lem:rq_local_unbounded}
For every $K \geq \zlow$ there exists a finite constant $C_r(K)$ such
that
\[
  |r_q(z)| \leq C_r(K)
  \qquad \text{for all } z \in [\zlow,K]
  \text{ and all } q \in \mathcal{Q}_\infty.
\]
One may take
\begin{equation}
\label{eq:CrK_unbounded}
  C_r(K)
  = (\|\psi\|_\infty + \mu)\,
    \frac{\bar q(K)}{(1+\gamma)\,\zlow^{1+\gamma}}
  + \mu\,\bar q(K)^{1/(1+\gamma)}
  + \frac{\mu\,\bar q(K)}{(1+\gamma)\,\zlow^{1+\gamma}}.
\end{equation}
\end{lemma}

\begin{proof}
Fix $K \geq \zlow$ and $z \in [\zlow,K]$.  By
Remark~\ref{rem:local_bounds_unbounded},
$q(s) \leq \bar q(K)$ for all $s \in [\zlow,K]$.
Bound each term of \eqref{eq:r_q_unbounded}:
\begin{align*}
  \left|(\psi(z)-\mu)\int_{\zlow}^z
    \frac{q(s)}{s^{2+\gamma}}\,ds\right|
  &\leq (\|\psi\|_\infty + \mu)\,\bar q(K)
    \int_{\zlow}^{K} s^{-(2+\gamma)}\,ds \\
  &\leq (\|\psi\|_\infty + \mu)\,
    \frac{\bar q(K)}{(1+\gamma)\,\zlow^{1+\gamma}}, \\
  \mu\,q(z)^{1/(1+\gamma)}
  &\leq \mu\,\bar q(K)^{1/(1+\gamma)}, \\
  \frac{\mu\,q(z)}{(1+\gamma)z^{1+\gamma}}
  &\leq \frac{\mu\,\bar q(K)}{(1+\gamma)\,\zlow^{1+\gamma}}.
\end{align*}
Summing the three bounds yields \eqref{eq:CrK_unbounded}.
\end{proof}

\begin{lemma}[Coercivity in $u_0$]
\label{lem:coercive_unbounded}
Under Assumptions~\ref{as:unbounded_QL},
\ref{as:psi_bounded_unbounded}, and
\ref{as:mu_interior_unbounded},
\[
  \sup_{q \in \mathcal{Q}_\infty} W(u_0,q) \to -\infty
  \qquad \text{as } u_0 \to \pm\infty.
\]
\end{lemma}
...
\emph{Step~(i): Localize.}
By Lemma~\ref{lem:localize_sign_sets_unbounded}, choose a
bounded set $\widetilde A_- \subset [\zlow,K]$ with
$\int_{\widetilde A_-} f > 0$ and
$\psi(z) \leq \mu - \delta_-$ on $\widetilde A_-$.

\emph{Step~(ii): On $\widetilde A_-$, the integrand grows
with $u_0$.}
Using \eqref{eq:g_decompose_unbounded}, for
$z \in \widetilde A_-$:
\[
  e^{-g_a(z)/\theta}
  = e^{-r_q(z)/\theta}\,
    e^{-(\psi(z)-\mu)u_0/\theta}.
\]
On $\widetilde A_-$: $\psi - \mu \leq -\delta_-$, so
$-(\psi - \mu)u_0 \geq \delta_- u_0$ when $u_0 > 0$.
Since $\widetilde A_- \subset [\zlow,K]$,
Lemma~\ref{lem:rq_local_unbounded} gives
$e^{-r_q/\theta} \geq e^{-C_r(K)/\theta}$.  Hence
\[
  e^{-g_a(z)/\theta}
  \geq e^{-C_r(K)/\theta}\,e^{\delta_- u_0/\theta}.
\]

\emph{Step~(iii): Bound the partition function from below.}
\[
  Z(u_0,q)
  = \int_{\calZ} e^{-g_a(z)/\theta} f(z)\,dz
  \geq e^{(\delta_- u_0 - C_r(K))/\theta}
      \int_{\widetilde A_-} f(z)\,dz.
\]

\emph{Step~(iv): Convert to $W$.}
Since $W = -\theta\log Z$ and $-\theta\log$ is decreasing:
\[
  W(u_0,q)
  \leq C_r(K) - \delta_- u_0
  - \theta\log\!\int_{\widetilde A_-} f(z)\,dz,
\]
which tends to $-\infty$ as $u_0 \to +\infty$.  The bound
is uniform over $q$ because $C_r(K)$ does not depend
on~$q$.

\emph{The case $u_0 \to -\infty$.}
Symmetric, using $\widetilde A_+$ and $\delta_+$.
\end{proof}


%..........................................................................
\subsubsection{Continuity and existence}

The remaining issue is to dominate the partition function on a strip
$|u_0| \leq M$.  On bounded support this was done with a two-sided
bound on $g_a$ (via $|r_q| \leq C_r$); on unbounded support a
one-sided \emph{lower} bound suffices.

\begin{lemma}[Uniform lower bound for $g_a$ on strips]
\label{lem:ga_lower_unbounded}
For every $M > 0$ there exists $C_g(M) > -\infty$ such that
\[
  g_a(z) \geq C_g(M)
  \qquad \text{for all } z \in [\zlow,\infty)
  \text{ and all } (u_0,q) \in \mathcal{A}_{q,\infty}(M).
\]
\end{lemma}

\begin{proof}
We use the economic decomposition
$g_a(z) = \psi(z)\,\calU(z) + \mu\,T(y(z))$
(equation~\eqref{eq:g_def}) and bound each term from below.

\emph{Step~(i): Lower bound on $\calU(z)$.}
Since $I_q(z) = \int_{\zlow}^z
q(s)\,s^{-(2+\gamma)}\,ds \geq 0$ (the integrand is
nonnegative):
\[
  \calU(z) = u_0 + I_q(z) \geq u_0 \geq -M.
\]

\emph{Step~(ii): Lower bound on $T(y(z))$.}
Admissibility gives $T'(y) \geq 0$ on path.  Since
$y(z)$ is nondecreasing (IC), $T(y(z))$ is
nondecreasing in $z$, so $T(y(z)) \geq T(y(\zlow))$
for all $z \geq \zlow$.  At the bottom type,
\[
  T(y(\zlow))
  = q(\zlow)^{1/(1+\gamma)} - u_0
  - \frac{q(\zlow)}{(1+\gamma)\zlow^{1+\gamma}}.
\]
Since $(u_0,q) \in \mathcal{A}_{q,\infty}(M)$ and
$q(\zlow) \leq \bar q(\zlow)$,
\[
  T(y(\zlow)) \geq -M
  - \frac{\bar q(\zlow)}{(1+\gamma)\zlow^{1+\gamma}}
  \eqqcolon T_{\min}(M).
\]

\emph{Step~(iii): Combine.}
If $\calU(z) \geq 0$, then $\psi(z)\calU(z) \geq 0$
(because $\psi \geq 0$), so
$g_a(z) \geq \mu T_{\min}(M)$.  If $\calU(z) < 0$, then
$\psi(z)\calU(z) \geq -\|\psi\|_\infty M$, hence
\[
  g_a(z) \geq -\|\psi\|_\infty M + \mu T_{\min}(M)
  \eqqcolon C_g(M). \qedhere
\]
\end{proof}

\begin{proposition}[Continuity of $W$ on coercive strips]
\label{prop:continuity_unbounded}
Under Assumptions~\ref{as:unbounded_QL} and
\ref{as:psi_bounded_unbounded}, $W$ is continuous on
$\mathcal{A}_{q,\infty}(M)$ for every $M < \infty$.
\end{proposition}

\begin{proof}
Let $(u_{0,n},q_n) \to (u_0,q)$ pointwise in
$\mathcal{A}_{q,\infty}(M)$.

\emph{Step~(i): Pointwise convergence of $g_{a_n}$.}
Fix $z \in [\zlow,\infty)$.  For each
$s \in [\zlow,z]$, we have $q_n(s) \to q(s)$ and, by
Remark~\ref{rem:local_bounds_unbounded},
$q_n(s) \leq \bar q(z)$.  Since
$s \mapsto \bar q(z)\,s^{-(2+\gamma)}$ is integrable on the
compact interval $[\zlow,z]$, Lemma~\ref{lem:DCT} gives
\[
  \int_{\zlow}^z \frac{q_n(s)}{s^{2+\gamma}}\,ds
  \to
  \int_{\zlow}^z \frac{q(s)}{s^{2+\gamma}}\,ds.
\]
The remaining terms in \eqref{eq:g_decompose_unbounded}
converge directly ($u_{0,n} \to u_0$, $q_n(z) \to q(z)$),
so $g_{a_n}(z) \to g_a(z)$ for every $z$.

\emph{Step~(ii): Integrable dominator.}
By Lemma~\ref{lem:ga_lower_unbounded},
$g_{a_n}(z) \geq C_g(M)$ for all $n$ and all $z$.  Hence
\[
  e^{-g_{a_n}(z)/\theta}\,f(z)
  \leq e^{-C_g(M)/\theta}\,f(z)
  \eqqcolon h(z).
\]
Since $f$ is a probability density:
$\int h = e^{-C_g(M)/\theta} < \infty$.

\emph{Step~(iii): Dominated convergence.}
The sequence $e^{-g_{a_n}/\theta}\,f$ converges pointwise
to $e^{-g_a/\theta}\,f$ (Step~(i)) and is dominated by the
integrable function $h$ (Step~(ii)).  By
Lemma~\ref{lem:DCT}:
\[
  Z(a_n) = \int_{\calZ} e^{-g_{a_n}(z)/\theta}f(z)\,dz
  \longrightarrow
  \int_{\calZ} e^{-g_a(z)/\theta}f(z)\,dz = Z(a).
\]
Since $Z(a) > 0$ (the integrand is strictly positive and
$f > 0$ on $\calZ$) and $\log$ is continuous:
$W(a_n) = -\theta\log Z(a_n) \to -\theta\log Z(a) = W(a)$.
\end{proof}

\begin{proposition}[Existence on unbounded support]
\label{prop:existence_unbounded}
Under Assumptions~\ref{as:unbounded_QL},
\ref{as:psi_bounded_unbounded}, and
\ref{as:mu_interior_unbounded}, $W$ attains a maximum on
$\mathcal{A}_{q,\infty}$.
\end{proposition}

\begin{proof}
By Lemma~\ref{lem:coercive_unbounded}, there exists
$M < \infty$ such that
$\sup_q W(u_0,q) < W(0,0)$ whenever $|u_0| > M$.
Therefore
\[
  \max_{\mathcal{A}_{q,\infty}} W
  = \max_{\mathcal{A}_{q,\infty}(M)} W.
\]
The set $\mathcal{A}_{q,\infty}(M)$ is sequentially compact
by Lemma~\ref{lem:Aq_compact_unbounded}, and $W$ is
continuous on it by
Proposition~\ref{prop:continuity_unbounded}.  Hence
Lemma~\ref{lem:weierstrass} implies that $W$ attains a
maximum.
\end{proof}


%..........................................................................
\subsubsection{Uniqueness and the induced tax schedule}

\begin{proposition}[Joint strict concavity on unbounded support]
\label{prop:unique_unbounded}
Under Assumptions~\ref{as:unbounded_QL},
\ref{as:psi_bounded_unbounded}, and
\ref{as:mu_interior_unbounded}, $W$ is jointly strictly concave in
$(u_0,q)$ on $\mathcal{A}_{q,\infty}$.  Consequently the maximizing
allocation $(u_0^*,q^*)$ is unique.
\end{proposition}

\begin{proof}
The argument follows Proposition~\ref{prop:unique_QL}, with
one additional step: we must verify that the partition
functions are finite on unbounded support.

Take $a_1 = (u_{0,1},q_1) \neq a_2 = (u_{0,2},q_2)$ in
$\mathcal{A}_{q,\infty}$ and $t \in (0,1)$.  Set
$a_t = ta_1 + (1-t)a_2$.

\emph{Step~(a): Concavity of $g_a$ in $(u_0, q)$.}
The map $q \mapsto q^{1/(1+\gamma)}$ is strictly concave,
while all remaining terms in $g_a$ are affine in $(u_0,q)$
(equation~\eqref{eq:g_uq}).  Hence
\[
  g_{a_t}(z) \geq t\,g_{a_1}(z) + (1-t)\,g_{a_2}(z)
\]
for every $z$, with strict inequality on
$\{z : q_1(z) \neq q_2(z)\}$.

\emph{Step~(b): Finiteness of partition functions.}
For each $i = 1, 2$, set $M_i = |u_{0,i}|$.
Lemma~\ref{lem:ga_lower_unbounded} gives
$g_{a_i}(z) \geq C_g(M_i)$ for all $z$, hence
\[
  Z(a_i) = \int_\calZ e^{-g_{a_i}/\theta}\,f
  \leq e^{-C_g(M_i)/\theta} < \infty.
\]

\emph{Step~(c): Exponentiate and apply H\"older.}
Since $x \mapsto e^{-x/\theta}$ is convex, Step~(a) gives
$e^{-g_{a_t}/\theta}
\leq (e^{-g_{a_1}/\theta})^t\,(e^{-g_{a_2}/\theta})^{1-t}$.
Integrating against $f$ and applying H\"older's inequality
(Lemma~\ref{lem:holder}, exponents $1/t$ and $1/(1-t)$;
valid because $Z(a_1), Z(a_2) < \infty$ by Step~(b)):
\[
  Z(a_t) \leq Z(a_1)^t\,Z(a_2)^{1-t}.
\]

\emph{Step~(d): Strict inequality.}
If $q_1 \neq q_2$ on a set of positive $f$-measure, the
concavity inequality in Step~(a) is strict on that set, so
the first inequality above is strict.
If $q_1 = q_2$ $f$-a.e.\ but $u_{0,1} \neq u_{0,2}$, then
Step~(a) is equality, but H\"older is strict because
$\exp(-(\psi(z)-\mu)(u_{0,1}-u_{0,2})/\theta)$ is not
constant $f$-a.e.\ by
Assumption~\ref{as:mu_interior_unbounded}.

\emph{Step~(e): Apply $-\theta\log$.}
Since $-\theta\log$ is strictly decreasing and
$-\theta\log(A^t B^{1-t}) = t(-\theta\log A)
+ (1-t)(-\theta\log B)$:
\[
  W(a_t) > t\,W(a_1) + (1-t)\,W(a_2). \qedhere
\]
\end{proof}

\begin{corollary}[Unique on-path tax schedule]
\label{cor:T_unique_unbounded}
Under the assumptions of Proposition~\ref{prop:unique_unbounded}:
\begin{enumerate}[label=(\roman*)]
\item the maximizing allocation $(u_0^*,q^*)$ exists and is unique;
\item the output schedule $y^* = (q^*)^{1/(1+\gamma)}$ is unique;
\item the on-path tax $T^*$ is uniquely determined at every income in
  the range of $y^*$, including bunching points;
\item off path, $T$ may be extended arbitrarily subject to no type
  deviating.
\end{enumerate}
All results are for the fixed-$\mu$ Lagrangian.
\end{corollary}
...
\begin{proof}
Existence is Proposition~\ref{prop:existence_unbounded},
uniqueness is Proposition~\ref{prop:unique_unbounded}, and
the construction of the on-path tax schedule is identical to
the argument in \S\ref{sec:existence}: if $q^*$ is constant
on a bunching interval, then $y^*$ is constant and $c^*$ is
constant there as well, so the value of $T^*(y) = y-c^*$ is
independent of the representing type.
\end{proof}


%..........................................................................
\subsubsection{Validity of the robust Saez formula}

Existence and uniqueness of the fixed-$\mu$ problem require no further
tail restriction beyond the boundedness of $\psi$ and the fact that
$f$ is a probability density.  The perturbation formula is different:
it requires differentiating through the integral defining $W$ with
respect to the tax schedule, which needs dominated convergence for
$|\partial_\alpha g_\alpha|\,e^{-g_\alpha/\theta}$.  On bounded
support these domination conditions are automatic; on
$\calZ = [\zlow,\infty)$ they must be imposed.

\begin{assumption}[Tail integrability for Saez perturbations]
\label{as:tail_formula_unbounded}
Let $T^*$ be the unique on-path tax schedule from
Corollary~\ref{cor:T_unique_unbounded}.  For every Saez band
perturbation $T_\alpha = T^* + \alpha\,\delta T$ centered at some
$y^* > 0$, the path $\alpha \mapsto g_\alpha(z)$ satisfies the three
hypotheses of Proposition~\ref{prop:envelope}: differentiability for
$f$-a.e.\ $z$, finiteness of the partition function
$Z(\alpha) = \int e^{-g_\alpha(z)/\theta} f(z)\,dz$ in a
neighborhood of $0$, and domination by some
$H \in L^1(f\,dz)$ of the form
\[
  |\partial_\alpha g_\alpha(z)| e^{-g_\alpha(z)/\theta} \leq H(z).
\]
\end{assumption}

\begin{remark}[Sufficient conditions]
\label{rem:sufficient_tail_unbounded}
For the Saez band perturbation with bounded $\delta T$,
$\delta T'$, bounded~$\psi$, and polynomially bounded
behavioral derivatives, a sufficient condition is that
$e^{-g_\alpha/\theta}\,f$ has enough tail decay to dominate
the polynomial growth---e.g., $f$ has a Pareto tail and
$\theta$ is finite (so the worst-case twist provides
stretched-exponential thinning).  Verifying these conditions
in specific calibrations is a separate task.
\end{remark}

\begin{proposition}[Robust Saez formula on unbounded support]
\label{prop:robust_saez_unbounded}
Let $T^*$ be the unique maximizing on-path tax schedule from
Corollary~\ref{cor:T_unique_unbounded}, and let
$\widetilde h^{\,*}$ denote the associated worst-case earnings
density.  Under Assumption~\ref{as:tail_formula_unbounded}, at every
income level $y^* > 0$ such that $T^*$ is differentiable at $y^*$ and
$\widetilde h^{\,*}(y^*) > 0$, we have
\begin{equation}
\label{eq:robust_saez_unbounded}
  \frac{T^{*\prime}(y^*)}{1-T^{*\prime}(y^*)}
  = \frac{1}{\zeta^c(y^*)}
    \cdot \frac{1}{y^*\,\widetilde h^{\,*}(y^*)}
    \int_{y^*}^{\infty}
    \bigl(1-\hat g^{\,*}(y)\bigr)
...
\end{equation}
Under quasilinear isoelastic preferences,
$\zeta^c(y) \equiv 1/\gamma$ and
$\hat g^{\,*}(y) = g^{\,*}(y) = \psi(z(y))/\mu$,
where $z(y)$ is the inverse of the on-path earnings map on
the relevant regular region.
\end{proposition}

\begin{proof}
Fix a Saez band perturbation around the candidate optimum $T^*$.  By
Assumption~\ref{as:tail_formula_unbounded}, the hypotheses of
Proposition~\ref{prop:envelope} hold on the unbounded support
$[\zlow,\infty)$, so the value function is Gateaux differentiable at
$T^*$ and its derivative is obtained by holding nature at the
worst-case distortion $m^*(T^*)$.  With $m$ fixed, the mechanical and
behavioral calculations of \S\ref{sec:saez_perturbation} are exactly
the same as before, except that the benchmark earnings density $h$ is
replaced by the worst-case density $\widetilde h^{\,*}$.  Therefore
\eqref{eq:dM_agg}--\eqref{eq:dB_agg} remain valid with
$\widetilde h$ replaced by $\widetilde h^{\,*}$.  Setting the first-order
effect equal to zero and letting the band width tend to zero
produces \eqref{eq:robust_saez_unbounded}.  Under quasilinear
preferences, income effects vanish, so
$\hat g^{\,*} = g^{\,*}$ and
$\zeta^c \equiv 1/\gamma$.
\end{proof}
...
\paragraph{Changes from the bounded case.}

\begin{center}
\renewcommand{\arraystretch}{1.4}
\begin{tabular}{lll}
\hline
\textbf{Step} & \textbf{Bounded (\S\ref{sec:existence})}
  & \textbf{Unbounded extension} \\
\hline
Feasible set
  & $q \leq \bar q$ (uniform)
  & $q(z) \leq \bar q(z) = z^{(1+\gamma)^2/\gamma}$ \\
Compactness
  & Helly on $[\zlow, \zbar]$
  & Diagonal Helly on $[\zlow, K_k]$, $k \to \infty$ \\
Coercivity
  & $|r_q| \leq C_r$ globally
  & $|r_q| \leq C_r(K)$ on bounded $\widetilde A_\pm
    \subset [\zlow, K]$ \\
Bound on $g_a$
  & $|g_a| \leq C_r + \|\psi{-}\mu\|_\infty M$ (global)
  & $g_a \geq C_g(M)$ (one-sided;
    Lem.~\ref{lem:ga_lower_unbounded}) \\
Continuity
  & DCT with constant dominator
  & DCT with $e^{-C_g/\theta}\,f$ \\
Uniqueness
  & H\"older (unchanged)
  & H\"older (plus finiteness check) \\
Saez formula
  & Automatic (bounded support)
  & Requires Asm.~\ref{as:tail_formula_unbounded} \\
\hline
New assumptions
  & $\zbar < \infty$
  & $\|\psi\|_\infty < \infty$;
    tail integrability for Saez \\
\hline
\end{tabular}
\end{center}

No additional tail condition on $\psi$ or $f$ is needed for
the fixed-$\mu$ existence/uniqueness result.  The two
ingredients that make this work are:
\begin{enumerate}
\item \textbf{Localization of the coercivity argument.}
  The sets $A_\pm$ where $\psi - \mu$ has definite sign
  can always be truncated to bounded subsets
  (Lemma~\ref{lem:localize_sign_sets_unbounded}), on which
  admissibility provides a local uniform bound on $r_q$
  (Lemma~\ref{lem:rq_local_unbounded}).

\item \textbf{One-sided bound on $g_a$.}
  Using $\psi \geq 0$, $\calU \geq -M$ (from
  $I_q \geq 0$), and $T \geq T_{\min}$ (from
  $T' \geq 0$): $g_a$ is bounded below by a constant on
  any strip $|u_0| \leq M$
  (Lemma~\ref{lem:ga_lower_unbounded}).  This gives
  $e^{-g_a/\theta} \leq e^{-C_g/\theta}$, and since $f$
  is a probability density, the constant is an integrable
  dominator.
\end{enumerate}

\begin{theorem}[The quasilinear unbounded-support extension]
\label{thm:unbounded_QL_extension}
Under Assumptions~\ref{as:unbounded_QL},
\ref{as:admissible},
\ref{as:psi_bounded_unbounded}, and
\ref{as:mu_interior_unbounded}, the fixed-$\mu$ robust problem on
$[\zlow,\infty)$ admits a unique maximizing allocation and a unique
on-path tax schedule.  If in addition
Assumption~\ref{as:tail_formula_unbounded} holds, then the unique
optimizer satisfies the robust Saez formula
\eqref{eq:robust_saez_unbounded} at every income level where $T^*$ is
differentiable and $\widetilde h^{\,*}$ is positive.
\end{theorem}

\begin{proof}
Existence follows from Proposition~\ref{prop:existence_unbounded},
uniqueness and on-path implementation from
Corollary~\ref{cor:T_unique_unbounded}, and validity of the Saez
formula from Proposition~\ref{prop:robust_saez_unbounded}.
\end{proof}

%--------------------------------------------------------------------------
\subsection{Saez formula beyond quasilinear  preferences}
\label{sec:clean_proof}
%--------------------------------------------------------------------------

The preceding subsections derived the robust Saez formula
(\S\ref{sec:value_function}--\ref{sec:three_factor}),
specialized it to the quasilinear isoelastic case
(\S\ref{sec:quasilinear}), and extended it to unbounded
support (\S\ref{sec:unbounded_QL}).
This subsection collects all hypotheses into a single
unified assumption set stated entirely in terms of
primitives, gives self-contained proofs of two key
technical results---the tail domination lemma and the
deformation cancellation---and then cascades through
progressively simpler preference specifications.

The goal is a clean, citable theorem:
Assumption~\ref{as:clean_main} $\Rightarrow$
Theorem~\ref{thm:clean_main}, with explicit
specializations (Corollaries~\ref{cor:clean_separable}--\ref{cor:clean_QL_bounded}).

\medskip\noindent\textbf{Notation for this subsection.}
To keep the proof self-contained we adopt concentrated
notation that slightly extends what precedes.  Write
\begin{equation}
\label{eq:clean_ell}
  \ell_T(z) := \psi(z)\,\calU(z; T) + \mu\,T(y(z; T))
\end{equation}
for the planner's per-type payoff (the quantity denoted
$g(z)$ in \S\ref{sec:natures_response}--\S\ref{sec:envelope};
we switch to $\ell$ here to avoid collision with the
welfare weight $g(y) = \psi U_c/\mu$).  As before,
$Z(T) := \int_\calZ e^{-\ell_T(z)/\theta}\,f(z)\,dz$ is the
partition function, $m^*(z;T) = e^{-\ell_T(z)/\theta}/Z(T)$
the worst-case likelihood ratio
(Proposition~\ref{prop:nature_br}), and
$\widetilde{f}(z) = m^*(z;T)\,f(z)$,
$\widetilde{h}(y) = \widetilde{f}(z(y))/y'(z(y))$
the worst-case densities
(Definition~\ref{def:worst_case}).

The welfare weight and adjusted welfare weight are as
in~\eqref{eq:ghat-pert}:
\[
  g(y) := \frac{\psi(z(y))\,U_c(z(y))}{\mu}, \qquad
  \hat g(y) := g(y) - \frac{T'(y)}{1-T'(y)}\,\eta(y).
\]
The compensated ETI $\zeta^c(y)$ and income effect
$\eta(y)$ are defined by the Slutsky
decomposition~\eqref{eq:slutsky-identity}.


%..........................................................................
\subsubsection{Assumptions}

\begin{assumption}[Primitives for the entropy-robust Saez formula]
\label{as:clean_main}
\leavevmode
\begin{enumerate}[leftmargin=2.8em]

\item[\textup{(U)}] \textbf{Preferences.}
$U \in C^2$ on an open domain in $\R_+ \times \R_+$, with $U_c > 0$, $U_n < 0$, $U_{nn} < 0$, and the single-crossing condition.

\item[\textup{(T)}] \textbf{Tax schedule.}
$T : \R_+ \to \R$ is $C^2$ with $0 \le T'(y) \le \bar\tau < 1$ for all $y \ge 0$.

\item[\textup{(M)}] \textbf{Global monotonicity.}
The behavioral mapping $z \mapsto y(z;T)$ is strictly increasing on $\calZ$.

\item[\textup{(F)}] \textbf{Type distribution.}
$f$ is a probability density on $\calZ = [\zlow,\infty)$ with $f(z) > 0$ for all $z \in \calZ$.

\item[\textup{($\psi$)}] \textbf{Welfare weights.}
$\psi : \calZ \to [0,\infty)$ is bounded and measurable.

\item[\textup{(C)}] \textbf{Continuity at $y^*$.}
$f$ is continuous at $z^* := z(y^*)$ and $\psi$ is continuous at $z^*$.

\item[\textup{(G)}] \textbf{Growth bound on adjusted welfare weight.}
There exist $C_g \ge 0$ and $q \ge 0$ such that for all $z \in \calZ$:
\begin{equation}
\label{eq:clean_ghat_bound}
  |1 - \hat g(y(z;T))| \le C_g(1+z^q).
\end{equation}

\item[\textup{(Tail)}] \textbf{Tail regularity.}
For the perturbation path $T_\alpha = T + \alpha\,\delta T_\varepsilon$ (the Saez band perturbation at $y^*$, defined in~\eqref{eq:clean_band} below), there exist constants $a,r,C,p > 0$, $b \in \R$, a bounded measurable set $A \subset \calZ$ with $\int_A f > 0$, and a constant $M_A$ such that for all $|\alpha| < \alpha_0$ and all $z \in \calZ$:
\begin{align}
  \ell_\alpha(z) &\ge az^r - b, \label{eq:clean_tail_lb} \\
  |\partial_\alpha \ell_\alpha(z)| &\le C(1+z^p), \label{eq:clean_tail_deriv} \\
  \ell_\alpha(z) &\le M_A \quad\text{for all }z \in A. \label{eq:clean_tail_local}
\end{align}
\end{enumerate}
\end{assumption}

\begin{remark}[Role of each condition]
\label{rem:clean_roles}
\leavevmode
\begin{itemize}[leftmargin=2em]
\item \textup{(U)} and \textup{(T)} give the Slutsky decomposition~\eqref{eq:slutsky-identity} and the household envelope theorem $\dot{\calU}_{\delta T}(z) = -U_c(z)\,\delta T(y(z))$.
\item \textup{(M)} is needed \emph{globally}, not just at $y^*$: the numerator of the formula integrates $(1-\hat g(y))\widetilde{h}(y)$ over $[y^*,\infty)$, and at each $y$ in that range, $\hat g(y)$ requires the inverse $z(y)$ and $\widetilde{h}(y) = \widetilde{f}(z(y))/y'(z(y))$ requires both the inverse and a positive derivative.  Three consequences: the global inverse $z : (y(\zlow),\infty) \to \calZ$ exists; the worst-case earnings density $\widetilde{h}$ is well-defined; and $\widetilde{h}(y^*) > 0$ (from $y'(z^*) > 0$, $f(z^*) > 0$, and $m^*(z^*;T) > 0$).
\item \textup{(C)} ensures that the function $b(y) := \frac{T'}{1-T'}\zeta^c y\widetilde{h}$ is continuous at $y^*$, via the deformation cancellation (Lemma~\ref{lem:clean_cancel}).
\item \textup{(G)} ensures integrability of the numerator: $\int_{y^*}^\infty |1-\hat g|\,\widetilde{h}\,dy < \infty$.  Combined with $\ell_T \ge az^r - b$ from \textup{(Tail)}: $|1-\hat g|\cdot m^*(z;T) \le C_g(1+z^q)e^{(b-az^r)/\theta}/Z(T)$, which is bounded.
\item \textup{(Tail)} ensures that differentiation under the integral sign is valid for the entropy envelope.  It is the only condition specific to unbounded support.
\end{itemize}
\end{remark}

\begin{remark}[Relation to earlier assumptions]
\label{rem:clean_relation}
On bounded support ($\calZ = [\zlow,\zbar]$ with $\zbar < \infty$), conditions~\textup{(G)} and~\textup{(Tail)} are vacuous: all functions involved are continuous on a compact domain, hence bounded (see Corollary~\ref{cor:clean_bounded}).  The content of Assumption~\ref{as:clean_main} beyond earlier assumptions is therefore entirely in these two tail conditions.  On unbounded support, they sharpen the high-level Assumption~\ref{as:tail_formula_unbounded} by tracing the domination requirement back to primitive bounds on $\ell_T$ and its derivative.
\end{remark}


%..........................................................................
\subsubsection{Tail domination}

\begin{lemma}[Tail domination]
\label{lem:clean_tail}
Under condition \textup{(Tail)} of Assumption~\ref{as:clean_main}:
\begin{enumerate}[label=(\roman*)]
\item $Z(T_\alpha) < \infty$ for all $|\alpha| < \alpha_0$;
\item $\inf_{|\alpha|<\alpha_0} Z(T_\alpha) > 0$;
\item there exists $H_0 \in L^1(f)$ such that $|\partial_\alpha \ell_\alpha(z)|\,e^{-\ell_\alpha(z)/\theta} \le H_0(z)$ for all $|\alpha| < \alpha_0$ and $f$-a.e.\ $z$.
\end{enumerate}
\end{lemma}

\begin{proof}
From~\eqref{eq:clean_tail_lb}: $e^{-\ell_\alpha(z)/\theta} \le e^{(b-az^r)/\theta} \le e^{b/\theta}$ (since $az^r \ge 0$).  Hence $Z(T_\alpha) \le e^{b/\theta}\int f = e^{b/\theta} < \infty$.

From~\eqref{eq:clean_tail_local}: $Z(T_\alpha) \ge \int_A e^{-\ell_\alpha/\theta}f \ge e^{-M_A/\theta}\int_A f =: \underline{Z} > 0$.

For (iii): $|\partial_\alpha \ell_\alpha|\,e^{-\ell_\alpha/\theta} \le C(1+z^p)\,e^{(b-az^r)/\theta}$.  The function $(1+z^p)e^{-az^r/\theta}$ is continuous on $[\zlow,\infty)$ and tends to $0$ as $z \to \infty$ (exponential beats polynomial when $a,r > 0$), hence bounded by some $M < \infty$.  Set $H_0(z) := Ce^{b/\theta}M$, a constant, which is in $L^1(f)$.
\end{proof}

\begin{remark}
Lemma~\ref{lem:clean_tail} provides the explicit dominator required by hypotheses~(ii)--(iii) of Proposition~\ref{prop:envelope}.  Combined with the differentiability of $\alpha \mapsto \ell_\alpha(z)$ from \textup{(U)} and \textup{(T)}, all three hypotheses hold, so the entropy envelope
\begin{equation}
\label{eq:clean_envelope}
  \left.\frac{d}{d\alpha}V(T_\alpha)\right|_{\alpha=0}
  = \int_\calZ \partial_\alpha \ell_\alpha(z)\big|_{\alpha=0}\,\widetilde{f}(z)\,dz
\end{equation}
follows from Proposition~\ref{prop:envelope}.  This is the only place where entropy differs from the general power-divergence case: because $V(T) = -\theta\log Z(T)$ is explicit, no Danskin theorem and no scalar dual variable are needed.
\end{remark}


%..........................................................................
\subsubsection{Directional derivative at fixed worst-case density}

\begin{lemma}[Directional derivative]
\label{lem:clean_directional}
Under \textup{(U)}, \textup{(T)}, \textup{(M)}, and \textup{(F)} of Assumption~\ref{as:clean_main}, for any perturbation direction $\delta T$:
\begin{equation}
\label{eq:clean_directional}
  \left.\frac{d}{d\alpha}V(T+\alpha\,\delta T)\right|_{\alpha=0}
  = \mu\int_0^\infty(1-\hat g(y))\,\delta T(y)\,\widetilde{h}(y)\,dy
  - \mu\int_0^\infty \frac{T'(y)}{1-T'(y)}\,\zeta^c(y)\,y\,\delta T'(y)\,\widetilde{h}(y)\,dy.
\end{equation}
\end{lemma}

\begin{proof}
By the entropy envelope~\eqref{eq:clean_envelope}, it suffices to compute $\partial_\alpha \ell_{T+\alpha\delta T}(z)|_{\alpha=0}$.  From~\eqref{eq:clean_ell}:
\[
  \partial_\alpha \ell_{T+\alpha\delta T}(z)\big|_0
  = \psi(z)\,\dot{\calU}_{\delta T}(z) + \mu\,\delta T(y) + \mu\,T'(y)\,\dot y_{\delta T}(y).
\]
The household envelope theorem (from \textup{(U)}): $\dot{\calU}_{\delta T}(z) = -U_c(z)\,\delta T(y(z))$.  Hence
\[
  \partial_\alpha \ell_{T+\alpha\delta T}(z)\big|_0
  = \mu(1-g(y))\,\delta T(y) + \mu\,T'(y)\,\dot y_{\delta T}(y).
\]
Substituting the Slutsky decomposition~\eqref{eq:slutsky-identity}:
\begin{align*}
  \partial_\alpha \ell_{T+\alpha\delta T}(z)\big|_0
  &= \mu\!\left(1-g(y)+\frac{T'(y)}{1-T'(y)}\,\eta(y)\right)\delta T(y)
  - \mu\,\frac{T'(y)}{1-T'(y)}\,\zeta^c(y)\,y\,\delta T'(y) \\
  &= \mu(1-\hat g(y))\,\delta T(y)
  - \mu\,\frac{T'(y)}{1-T'(y)}\,\zeta^c(y)\,y\,\delta T'(y),
\end{align*}
using the definition of $\hat g$ in~\eqref{eq:ghat-pert}.  Integrating against $\widetilde{f}(z)\,dz$ and changing variables from $z$-space to $y$-space via \textup{(M)} and \textup{(F)} gives~\eqref{eq:clean_directional}.
\end{proof}


%..........................................................................
\subsubsection{The deformation cancellation}

\begin{lemma}[Deformation cancellation]
\label{lem:clean_cancel}
Under \textup{(U)}, \textup{(T)}, \textup{(F)}, and \textup{(M)} of Assumption~\ref{as:clean_main}: the product $\Sigma(y) := \zeta^c(y)\cdot y\cdot\widetilde{h}(y)$ does not depend on $T''(y)$.  For separable $U = u(c) - v(n)$:
\begin{equation}
\label{eq:clean_Sigma_sep}
  \Sigma(y) = \frac{u'(c(y))\,\widetilde{f}(z(y))\,z(y)^2}{v''(n(y))\,n(y) + v'(n(y))}.
\end{equation}
Consequently, $b(y) = \frac{T'}{1-T'}\Sigma(y)$ depends on $T$ only through $T'(y)$ and $T(y(\cdot))$ (via $m^*$ in $\widetilde{f}$).
\end{lemma}

\begin{proof}
Write $\Phi(y,z,T',c) = 0$ for the household FOC at fixed type $z$, marginal rate $T'$, and consumption $c$.  The implicit function theorem gives $\zeta^c = \Phi_\tau/(y\,\mathcal{D}')$ where $\mathcal{D}'$ contains $T''$, and $\widetilde{h} = \widetilde{f}/y'$ with $y' = -\Phi_z/\mathcal{D}'$.  Hence $\Sigma = \frac{\Phi_\tau}{y\,\mathcal{D}'}\cdot y\cdot\frac{\widetilde{f}}{-\Phi_z/\mathcal{D}'} = \frac{\Phi_\tau\,\widetilde{f}}{-\Phi_z}$.  The second-order condition $\mathcal{D}'$ cancels completely.

For separable preferences $U = u(c) - v(n)$, the FOC is $v'(y/z)/z = u'(y-T(y))(1-T'(y))$.  The partial derivatives at fixed $(y,T',c)$ are $\Phi_\tau = u'(c)$ and $\Phi_z = -(v''(n)\,n+v'(n))/z^2$.  Substituting gives~\eqref{eq:clean_Sigma_sep}.
\end{proof}

\begin{remark}
The cancellation explains why \textup{(C)} (continuity of $f$ and $\psi$ at $z^*$) is sufficient for continuity of $b(y^*)$: the dependence on $T''$ drops out of $\Sigma$, and the remaining factors---$T'$, $u'(c)$, $v''(n)n + v'(n)$, $\widetilde{f}(z)$---are all continuous at $z^*$ under the stated conditions.
\end{remark}


%..........................................................................
\subsubsection{The Saez band perturbation and main theorem}

Fix $y^* > 0$ and $\varepsilon > 0$.  The band perturbation is:
\begin{equation}
\label{eq:clean_band}
  \delta T_\varepsilon(y) := d\tau \times
  \begin{cases}
    0, & y < y^*, \\
    y - y^*, & y \in [y^*,y^*+\varepsilon], \\
    \varepsilon, & y > y^*+\varepsilon,
  \end{cases}
  \qquad d\tau > 0.
\end{equation}
Then $\delta T_\varepsilon'(y) = d\tau\,\mathbf{1}_{[y^*,y^*+\varepsilon]}(y)$ for a.e.\ $y$.  This is the same perturbation as in \S\ref{sec:saez_perturbation}.

\begin{theorem}[Entropy-robust Saez formula from primitives]
\label{thm:clean_main}
Let $T^*$ be an interior optimum of $V(T)$ in~\eqref{eq:maxmin}.  Under Assumption~\ref{as:clean_main} at $T = T^*$ and $y^* > 0$:
\begin{equation}
\label{eq:clean_saez}
  \boxed{
  \frac{T^{*\prime}(y^*)}{1 - T^{*\prime}(y^*)}
  =
  \frac{1}{\zeta^c(y^*)\,y^*\,\widetilde{h}(y^*)}
  \int_{y^*}^\infty (1-\hat g(y))\,\widetilde{h}(y)\,dy.
  }
\end{equation}
\end{theorem}

\begin{proof}
Apply Lemma~\ref{lem:clean_directional} with $\delta T = \delta T_\varepsilon$.  Since $T^*$ is interior, the directional derivative vanishes:
\begin{equation}
\label{eq:clean_FOC}
  0 = \mu\int_0^\infty (1-\hat g)\,\delta T_\varepsilon\,\widetilde{h}\,dy
  - \mu\int_0^\infty b(y)\,\delta T_\varepsilon'(y)\,dy,
\end{equation}
where $b(y) := \frac{T'(y)}{1-T'(y)}\,\zeta^c(y)\,y\,\widetilde{h}(y)$.

\medskip
\noindent\textbf{Mechanical term.}
Write $k(y) := (1-\hat g(y))\,\widetilde{h}(y)$.  Using~\eqref{eq:clean_band} and adding and subtracting $\varepsilon\,d\tau\int_{y^*}^{y^*+\varepsilon}k\,dy$:
\[
  \int_0^\infty k(y)\,\delta T_\varepsilon(y)\,dy
  = \varepsilon\,d\tau\int_{y^*}^\infty k(y)\,dy + O(\varepsilon^2\,d\tau).
\]
The remainder is $O(\varepsilon^2\,d\tau)$ because
$\int_{y^*}^{y^*+\varepsilon}|k| = O(\varepsilon)$ by local
integrability of $k$ near $y^*$.  (Integrability of $k$ on
$[y^*,\infty)$ follows from \textup{(G)} and
\textup{(Tail)}: $|k(y(z))|\cdot m^*(z;T) \le
C_g(1+z^q)e^{(b-az^r)/\theta}/Z(T)$, which is bounded.)

\medskip
\noindent\textbf{Behavioral term.}
Since $\delta T_\varepsilon'(y) = d\tau\,\mathbf{1}_{[y^*,y^*+\varepsilon]}(y)$:
\[
  \int_0^\infty b(y)\,\delta T_\varepsilon'(y)\,dy = d\tau\int_{y^*}^{y^*+\varepsilon}b(y)\,dy.
\]
By \textup{(C)} and the deformation cancellation (Lemma~\ref{lem:clean_cancel}), $b$ is continuous at $y^*$.  Hence
\[
  \int_0^\infty b\,\delta T_\varepsilon'\,dy = \varepsilon\,d\tau\,b(y^*) + o(\varepsilon\,d\tau).
\]

\medskip
\noindent\textbf{Conclusion.}
Substituting into~\eqref{eq:clean_FOC}, dividing by $\mu\,\varepsilon\,d\tau > 0$, and sending $\varepsilon \downarrow 0$:
\[
  0 = \int_{y^*}^\infty (1-\hat g)\,\widetilde{h}\,dy - b(y^*).
\]
Since $\widetilde{h}(y^*) > 0$ (by \textup{(M)} and \textup{(F)}: $m^*(z^*;T) > 0$, $f(z^*) > 0$, $y'(z^*) > 0$), rearranging gives~\eqref{eq:clean_saez}.
\end{proof}

\begin{remark}[Rawlsian weights]
\label{rem:clean_rawlsian}
The most common specification violating \textup{(C)} is Rawlsian weights $\psi(z) = \mathbf{1}_{z \le \hat z}$.  At any $y^* \neq y(\hat z)$, $\psi$ is locally constant, so \textup{(C)} holds and the theorem applies.  At $y^* = y(\hat z)$, $b$ has a bounded jump discontinuity and the band perturbation from above gives a one-sided necessary condition with $b(y(\hat z)^+)$.  The formula holds at every income level except the single point $y(\hat z)$.
\end{remark}
...
\end{align}
exactly as in \eqref{eq:dM-general}--\eqref{eq:dB-general}, but now with $\hat{g}$ depending on both $y$ and $\gamma$.
...
This appendix derives the Hamiltonian optimality conditions used throughout the notes from first principles, via a perturbation (variational) argument. We begin with the necessary mathematical background on optimization in function spaces, following Luenberger (1969, Chapter~7).

\subsection{Optimization in function spaces}
\label{sec:functional_optimization}

The planner's problem---whether in the Hamiltonian formulation (Section~\ref{sec:hamiltonian}) or the perturbation approach (Section~\ref{sec:perturb-setup})---is an optimization over \emph{functions} (the tax schedule $T(\cdot)$ or the allocation $(c(\cdot), y(\cdot))$). The relevant mathematical framework is the calculus of variations in Banach spaces.

\begin{definition}[Gâteaux derivative]
\label{def:gateaux}
Let $X$ be a Banach space and $J: X \to \R$ a functional. The \textbf{Gâteaux derivative} of $J$ at $x \in X$ in the direction $\delta x \in X$ is
\[
  DJ(x;\, \delta x) = \lim_{\epsilon \to 0} \frac{J(x + \epsilon\,\delta x) - J(x)}{\epsilon},
\]
whenever this limit exists. If $DJ(x;\, \delta x)$ exists for all $\delta x \in X$ and is linear and continuous in $\delta x$, we say $J$ is \textbf{Gâteaux differentiable} at $x$.
\end{definition}

The Gâteaux derivative is the infinite-dimensional analogue of the directional derivative. In the tax problem, $X$ is a space of tax schedules (e.g., piecewise $C^1$ functions), $x = T$, and $\delta x = \delta T$ is the perturbation direction.

\begin{proposition}[Necessary condition for an extremum]
\label{prop:gateaux_necessary}
Let $J: X \to \R$ be Gâteaux differentiable at $x^* \in X$. If $x^*$ is a local extremum of $J$ over an open subset of $X$, then
\begin{equation}
\label{eq:gateaux_necessary}
  DJ(x^*;\, \delta x) = 0 \qquad \text{for all } \delta x \in X.
\end{equation}
\end{proposition}

\begin{proof}
Fix any direction $\delta x \in X$. Define $\phi(\epsilon) = J(x^* + \epsilon\,\delta x)$, a real-valued function of the scalar $\epsilon$. Since $x^*$ is a local extremum of $J$, $\phi$ has a local extremum at $\epsilon = 0$. By the standard one-dimensional necessary condition, $\phi'(0) = 0$. But $\phi'(0) = DJ(x^*;\, \delta x)$ by definition.
\end{proof}

\begin{remark}[Constrained problems]
When the feasible set is not open---for instance, if the tax schedule must satisfy $T'(y) \in [0,1]$---the necessary condition \eqref{eq:gateaux_necessary} holds only for directions $\delta x$ that point into the feasible set. For directions that would violate a binding constraint, the equality is replaced by an inequality (a variational inequality). In the tax problem, this arises at bunching points where the monotonicity constraint $y'(z) \geq 0$ binds.
\end{remark}

\begin{remark}[Sufficient conditions]
The Gâteaux condition \eqref{eq:gateaux_necessary} is necessary but not sufficient. Sufficiency requires additional structure---typically concavity of $J$ (or convexity for minimization). In the tax problem, sufficiency is ensured by the concavity of $U$ in $(c, n)$ and the single-crossing property, which together guarantee that the first-order conditions characterize the global optimum.
\end{remark}

\begin{remark}[Application to the perturbation approach]
In Section~\ref{sec:perturb-setup}, the functional is the planner's Lagrangian $\mathcal{L}(T)$ and the direction is an arbitrary perturbation $\delta T$. Equation \eqref{eq:dL-total-1D} computes $D\mathcal{L}(T;\, \delta T)$, and the optimality condition $D\mathcal{L}(T^*;\, \delta T) = 0$ for all $\delta T$ is exactly Proposition~\ref{prop:gateaux_necessary}. The Saez perturbation chooses a specific family of directions (marginal rate increases in narrow bands), but the condition must hold for \emph{every} direction, which is what pins down the tax schedule pointwise.
\end{remark}
