...
To avoid misunderstandings, it is important to say what I mean by 'model', 'rational expectations model', 'rational expectations econometrics',  and 'wrong beliefs'.  By a 'model' I mean a joint probability distribution over a sequence indexed by a known vector of parameters. A 'rational expectations model' is one in which all of the agents inside the model share the model with the economic theorist who created it.  By `rational expectations econometrics' I mean that the econometrician shares the shares the model with the economic theorist.  By `wrong beliefs' I mean that some of the people  inside a model uses a different model that do the theorist and  the econometrician. Thus, 'wrong beliefs' are relative to a baseline model that an econometrician has somehow inferred from historical data via a process of induction that I won't describe in this paper.   
...
misspecification.\footnote{\textbf{Tom: add cite to Ellison paper.}}  Section~\ref{sec:robust} develops a theory in which
...
\footnote{\textbf{Tom: add reference to IEEE paper.}}
...
\item \textbf{Costs of aggregate fluctuations.}  Welfare assessments of
      ...
      pessimistic probability model that amplifies downside risk.\footnote{\textbf{Tom: also add reference to BHS.}}
