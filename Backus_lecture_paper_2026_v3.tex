...
\large Three Martingales and More\thanks{The research presented in this paper was  the basis for
the 2017 inaugural David Backus Memorial Lecture.
Dave was a pioneer in the study of affine term structure models and
international risk sharing.
His 2001 paper \citep{BFT2001} with Foresi and Telmer on affine models of currency
...
\begin{abstract}
%This paper explores how asset prices confound the consequences  of risk aversion and mistaken beliefs. 
 A single  mathematical object connects
risk-neutral pricing, subjective belief distortions, and concerns about
model misspecification, namely, a multiplicative martingale increment that defines a likelihood ratio.
 I describe  likelihood ratios that:
(i)~encode a representative investor's risk aversion, following
Lucas and Hansen; (ii)~represent mistaken beliefs of professional
forecasters, following \citet{PSS2015}; and (iii)~capture fears of model misspecification, following
\citet{HSHS2020}.  By using  a tilted discounted entropy
ball that accommodates concerns about particular parametric models
featuring long-run risk, the third likelihood ratio can generate  state-dependent uncertainty
prices that can explain hitherto puzzling movements in the term structure of interest rates.

\medskip

\noindent \textbf{Keywords:} Risk, uncertainty, likelihood ratios, relative entropy, interest rates.

\medskip

\noindent \textbf{JEL Classification:} E62, H21, H63
\end{abstract}
...
This paper studies some    long-standing puzzles about the term structure of interest rates. By ``puzzle'' I mean a feature of a joint distribution of macroeconomic variables and asset returns that is  implied by a ``standard'' benchmark model but  that is contradicted by U.S.\ time series data.\footnote{Thus, a ''puzzle'' is always relative to a baseline model.}  Relative to a ''standard'' asset pricing model like \citet{Lucas1978} or \citet{cox1985theory}, three patterns qualify as puzzles: 1) the nominal yield curve usually slopes upward; 2) the long-minus-short yield spread narrows before recessions and widens after them; and 3) long-term bond yields are nearly as volatile as short-term yields even though, under a rational expectations version of the expectations theory of the term structure, they should be much smoother averages of expected future short rates---this is what \citet{Shiller1979} called the \emph{volatility puzzle}.\footnote{For influential research about the expectations theory of the term structure of interest rates before rational expectations was imposed, see \citet{bierwag1967model} and \citet{meiselman1961term}.}

\citet{hansen1981exact} tested cross-equation restrictions on vector autoregressions implied by the rational expectations theory of the term structure.\footnote{Also see \citet{Sargent1972,Sargent1979} and \citet{HansenSargent1991}, which
reprinted \citet{hansen1981exact}.} They found some support for the theory's implications for conditional means, but also evidence of the difficulties with conditional second moments that Shiller emphasized. To resolve the volatility puzzle, risk prices---or something observationally equivalent---must be large, volatile, and state-dependent. What generates those state-dependent risk prices? Risk aversion? Wrong beliefs? Fears of model misspecification? Disentangling these possible sources is the topic of this paper.

To set the stage, I say what I mean by 'model', 'rational expectations model', 'rational expectations econometrics',  and 'wrong beliefs'.  A (statistical)  'model' is  a joint probability distribution over a sequence of random variables indexed by a  vector of known parameters. In a  'rational expectations model', all of the agents inside the model share the model with the economic theorist who created it.   `Rational expectations econometrics'  means that the econometrician shares the shares the model with the economic theorist.   `Wrong beliefs'  means that some of the people  inside the  model  don't use  the econometrician's model. Thus, 'wrong beliefs' are relative to a  model that an econometrician has somehow inferred from historical data via a process of induction that I won't describe in this paper.  Below we'll sometimes refer to the econometrician's model  as the ``baseline model'' or the econometrician's ``approximating model''.

In asset pricing data, consequences  of investors' \emph{risk aversion} and \emph{wrong beliefs} are
confounded.  In a rational expectations model with a
risk-averse representative investor, higher mean returns compensate for
higher risks.  But if the representative investor holds wrong beliefs, belying the  rational expectations ``communism-of-beliefs ''assumption,
observed average returns depend on \emph{both} risk aversion \emph{and}
misunderstood return distributions.  Wrong beliefs can generate what, from the perspective of an econometrician's approximating model, look
like ``stochastic discount factor shocks.'' Can such ``shocks'' explain the observed countercyclical behavior of risk prices?


This paper organizes ideas around a single concept: the
\emph{likelihood ratio}, a non-negative random variable with unit mean
that twists one probability distribution into another.
Much of the analysis draws on two contributions:
\citet{HSHS2020}, who construct
twisted probability models from tilted discounted entropy balls to
price model uncertainty, and \citet{Szoke2022}, who estimates a
...
ratio processes $\{M_t = \prod_{s=0}^t m_s\}_{t=0}^\infty$ formed from (i)~risk prices $\lambda_t$,
(ii)~belief distortions $w_t^*$, and (iii)~worst-case model distortions
$\tilde w_t$.  The ``and more'' in my subtitle refers to $\overline w_t$, a parametric
long-run risk worry that tilts an entropy ball.

The rest of this paper proceeds as follows.  Section~\ref{sec:risks-and-beliefs}
sets up astatistical framework in which a likelihood ratio twists
...
Section~\ref{sec:identification} discusses an statistical  parameter identification issue that formalizes how
asset pricing data confound risk-aversion and wrong beliefs:
the same likelihood ratio can be interpreted as risk aversion or as
belief distortion relative to the econometrician's approximating model. Section~\ref{sec:PSS} describes an approach of
\citet{PSS2015}, who use survey data on
professional forecasters to decompose the likelihood ratio into a
smaller risk price and a belief distortion.
Section~\ref{sec:RE-limits} pauses to discuss how  rational expectations
may not be a good approximation when persistent dynamics make convergence
of learning occur slowly.
...
joint probability distributions matter for macroeconomics. Appendix~\ref{sec:multiplier} describes the multiplier preference formulation of a dubious agent's problem.
...
structure unchanged.  The  framework sets forth vocabulary and notation that will recur
throughout the paper.

Let $\varepsilon$ denote a vector of risks to be borne and priced.
...
a convenient measure of the statistical distance between the two models.
 ...
 Depending on context,  the vector $\lambda$
...
stock prices satisfy \citep{Shiller1979}:
\[
   p_t = \exp(-r_t)\, E_t\bigl( p_{t+1} + d_{t+1}\bigr).
\]
The expectations theory of the term structure of interest rates
(\citealt{Hicks1939}--\citealt{Shiller1979}) prices a zero-coupon risk-free claim to one dollar at
time $t+n$ as:
\begin{align}
p_t(1) &= \exp(-r_t), \notag\\
...
With this device, modern asset pricing (\citealt{Lucas1978} for stocks,
\citealt{DS2000}, \citealt{cox1985theory}, and \citealt{BZ1994} for bonds) takes the form:
\begin{align}
...
predictions.  This insight was articulated by \citet{HST1999} and \citet{PSS2015}.
...
bring in additional data.  \citet{PSS2015}
...
\citet{PSS2015} (henceforth PSS) exploit data on
...
risk prices $\lambda_t$ that are the sum of two parts:
\begin{itemize}
...
An intriguing interpretive question lurks here. What questions do professional forecasts think that
they are answering when they respond to surveys. Are they reporting their best guess of the future, or the scenario
...
misspecification.\footnote{\citet{ellison2012defense} applied this idea to interpreting responses of  Federal Reserve officials to  questions about the forecasts of macroeconomic outcomes that rationalize their decisions.}  Section~\ref{sec:robust} develops a theory in which
...
\footnote{\citet{Sargent2008} discusses the intellectual history of rational
...
to \citet{LucasSargent1981} lays out the case for rational expectations
...
\citet{BY2004}, the required sample lengths can exceed a
...
the set of models that the dubious agent contemplates.\footnote{This property of
 discounted entropy 
is what motivated \citet{hansen1995discounted} to introduce it.}
...
that the data demand.  This section shows how \citet{HSHS2020}
...
To generate state-dependent uncertainty prices, \citet{HSHS2020}
...
\citet{BY2004} mix long-run risk (LRR) with Epstein--Zin (EZ)
...
as a `bazooka' because of the powerful way it raises market prices of risk.}   LRR is statistically difficult to detect and estimate, but
...
established by \citet{HS2001} and \citet{Maenhout2004}, implies
...
Inspired by \citet{BY2004}, an agent fears particular long-run
risks expressed by
\[
...
Sz\H{o}ke's (\citeyear{Szoke2022}) estimation of the tilted entropy ball model, which
...
\hline\\[-6pt]
\citealt{Lucas1978}  &  no & no & no \\[4pt]
Epstein--Zin with LRR & maybe & yes & no \\
\quad (\citealt{PS2007}, \citealt{HS2001}) & & & \\[4pt]
\citealt{PSS2015} &  built-in  &  built-in & yes \\[4pt]
\citealt{Szoke2022} & YES  & yes & yes \\[4pt]
\end{tabular}
\end{center}
...
the equations of a vector autoregression.\footnote{See \citet{LucasSargent1981}.}  
...
\citet{HansenSargent1991} provided systematic methods for deriving and
...
I have used likelihood ratios to organize three distinct explanations for observed risk prices---risk aversion, mistaken beliefs, and fears of model misspecification---and has shown how the third, properly formalized, can account for puzzling term-structure dynamics.
The question posed in my  title matters because the same empirical regularities can call for different responses  depending on which interpretation is correct. Joint probability distributions of asset prices and macroeconomic fundamentals therefore matter throughout macroeconomics.
...
\item \textbf{Costs of aggregate fluctuations.}  Welfare assessments of
      ...
      \citet{Lucas1987} calculated the welfare cost of business cycles at
      ...
      by one or two orders of magnitude (see \citep{HST1999}), because the agent evaluates fluctuations under a
      pessimistic probability model that amplifies downside risk.\footnote{ See \citet{barillas2009doubts} for a  quantitive exploration of this idea.}
\item \textbf{Consumption Euler equations.}  The ``New Keynesian IS
      ...
      Whether those adjustments reflect risk aversion, wrong beliefs, or model uncertainty affects how monetary policy transmits to real activity through intertemporal substitution.
\item \textbf{Optimal taxation and government debt management.}
       ...
       matters for optimal fiscal policy.\footnote{See \citet{Karantounias2013} and \citet{FerrierKarantounias2019}.}
...
\item \textbf{Long-run risk and secular stagnation.}  The Bansal--Yaron
      ...
      very differently than one who does not.\footnote{See \citet{hansen2010fragile}.}
\end{itemize}
...
each component--- thus matters for both positive and normative
macroeconomics. Dave Backus spent a lot of time teaching us how to  improve connections  between theory and evidence. I wish he were still here. In some important ways, he still is.   
...
\bibliography{Backus_lecture_paper_2026}
