...
I interpret the episode through the lens of modern machine learning:
...
illuminates both the promises  and the limits of theory-free AI for scientific
discovery.
\end{abstract}
%
%\tableofcontents
...
effect---in a paper that would earn him the 1921 Nobel Prize.  Einstein's paper 
posited a falsifiable linear equation relating the stopping
potential of emitted electrons to the incident frequency, with slope $h/e$---a
prediction that Millikan confirmed to 0.5\% in 1916.  It also 
...
reasoning to the general Schr\"odinger wave function.  The paper that launched
the precision measurement effort documented here thus led to Born's rule.
Einstein spent the rest of his life opposing it.
...
\begin{enumerate}
  ...
  \item \textbf{Unintended conceptual consequences.}  The light-quantum
    hypothesis makes the classical wave intensity $|E|^2$, reinterpreted as a
    photon density, a probability density.  Born traced his probabilistic
    interpretation of the Schr\"odinger wave function to this precedent
    \citep{Born1926}.
\end{enumerate}

Einstein's quantum hypothesis committed him to a view in which $|E|^2$ governs
the statistical behavior of photons rather than the energy of a continuous
wave.  In private correspondence Einstein called this a \emph{Gespensterfeld}
(ghost field): a wave that guides probabilities without itself being
physically real in the classical sense.  Born's 1926 paper \citep{Born1926}
formalised that idea for the Schr\"odinger wave function.  Einstein won the
1921 Nobel Prize for the equation.  Born won the 1954 Nobel Prize for the
probabilistic interpretation that the equation implied.
...
confined to a 60\,nm UV range and gave unreliable estimates.
...
changed with surface age, absorbed gas, and temperature.  The term $v_c$
enters additively and shifts all stopping potentials by the same amount.  A
$v_c$ that stays constant during a run leaves $\beta$ unaffected.  Millikan verified this by monitoring $v_c$
...
\noindent $R^2 = 1 - \hat\sigma^2 n/(n-1)/S_{VV} \approx 0.99999$.
...
nuisance parameter.  The intercept $\alpha = -W/e + v_c$ of \eqref{eq:reg}
conflates the work function $W/e$ with the apparatus term $v_c$.
...
All pre-Millikan estimates are biased downward by
10--47\%.  The bias is attributable to stray short-wavelength light
...
ones that statistical learning theory formalises.  This section states the
...
\approx 5{,}700$ \citep{Vapnik1998}.  Reliable generalisation to unseen
frequencies then requires $n\gtrsim 5{,}700$ labeled training examples for
1\% accuracy with high probability.  The
...
A linear, a quadratic, and a square-root form fit equally well over a 60\,nm
UV band ($\Delta\nu \approx 0.7\times10^{14}$\,Hz).  The DNN has no mechanism
...
orders of magnitude.  He did so before a single labeled example was
available.
...
$n$.  ATI extends hypothesis class~\eqref{eq:hyp_class} to allow non-unit
photon multiplicity; the slope remains
...
measures $h/e$ to 0.5\%.  It does not distinguish the quantised-field
...
coincidences.  The antibunching experiment rules out the semiclassical
hypothesis of \citeauthor{Lamb1969} and establishes that the radiation field
is quantised.
...
discriminating experiment, a chain of hypothetico-deductive loops
(Table~\ref{tab:loop}) whose links are forged by theory, not by data alone.
