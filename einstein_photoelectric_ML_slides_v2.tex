...
% Style: default theme, matching block_chain_jones.tex
...
\title{How Einstein Induced the Photoelectric Effect: \\ Machine Learning}
...
\textbf{Three acts:}
\begin{enumerate}
  \item \textbf{Theory} (1905): Einstein proposes the model that
    defines the hypothesis class
  \item \textbf{Data quality} (1887--1916): experimenters spend
    29 years collecting clean training data
  \item \textbf{Estimation} (1916): Millikan runs the regression
    and recovers Planck's constant to 0.5\%
\end{enumerate}

\bigskip
\small The episode illustrates \emph{both} the power of theory-guided
...
$v_c$: EMF (electromotive force) between emitter
...
\begin{itemize}
  \item[$\checkmark$] \textbf{Fresh surfaces:} electromagnetic knife shaved the alkali-metal cylinder in vacuum---no oxidation, no work-function contamination
  \item[$\checkmark$] \textbf{Spectrally pure lines:} mercury arc with filters to exclude UV companions---eliminates stray-light bias on $V_0$
  \item[$\checkmark$] \textbf{Wide frequency range:} 5461\,\AA\ to 2535\,\AA\
    ($\Delta\nu = 6.3\times10^{14}$\,Hz, nearly a factor of two)
  \item[$\checkmark$] \textbf{Controlled nuisance:} Kelvin double-balance measured $v_c$ continuously; verified stable to $\pm 0.01$\,V over 9 hours
\end{itemize}
...
\end{column}
\begin{column}{0.45\textwidth}
\begin{block}{Lesson from Kepler and Newton}
Einstein recovered a first-class "Kepler stage" model in sense
of Koopmans (1947)
...
%----------------------------------------------------------------------
\begin{frame}[shrink=12]{The Hypothesis Class: Where Einstein Differs from atheoretical ML}
...
\end{column}
\begin{column}{0.45\textwidth}
\begin{alertblock}{What atheoretical ML Misses}
...
\bigskip
\begin{block}{Parameter Space  Reduction Einstein Achieved}
...
\textbf{Millikan 1916} & 5            & Sufficient
  & Very insufficient & Interpolates; no inference \\[2pt]
...
\bigskip
\begin{block}{Where DNNs Prevail}
...
%----------------------------------------------------------------------
\begin{frame}[shrink=5]{A Broader Lesson: The Hypothetis-Deduction Loop}
...
\smallskip
\textbf{\alert{The human contribution---theory construction---remains
indispensable.}}
\end{block}
...
\bigskip
\textbf{Max Born (1926)} applied Einstein's insight to interpret  Schr\"odinger's wave function:
\[
  \boxed{|\psi|^2 = \text{probability density}}
\]
\textbf{Born's rule} ---   quantum mechanics.
\end{column}
\begin{column}{0.45\textwidth}
...
%----------------------------------------------------------------------
\begin{frame}[shrink=5]{Summary and Contributions}

\begin{block}{The Thesis, Revisited}
Einstein was doing statistics with a tightly parameterized model  in 1905.
ML systems are yet to automate a key step: \textbf{specifying the
hypothesis class}.
\end{block}
...
\begin{enumerate}
  ...
  \item \textbf{Confirming an equation $\neq$ endorsing the model.}
    Millikan confirmed the slope to 0.5\% while publicly rejecting the
...
\begin{itemize}
  ...
  \item Millikan, R.\,A.\ (1916). ``A Direct Photoelectric Determination of
    Planck's $h$.'' \textit{Phys.\ Rev.}, 7(3), 355--388.
  ...
  \item Koopmans, T.\,C.\ (1947). ``Measurement without Theory.''
    \textit{The Review of Economics and Statistics}, 29(3), 161--172.
\end{itemize}
