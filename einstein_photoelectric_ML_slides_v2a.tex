...
%Three acts:
\begin{enumerate}
  \item \textbf{Theory} (1905): Einstein proposes a manifold of statistical models (i.e., an ``hypothesis class'')
  \item \textbf{Data} (1887--1916): experimenters 
    collect good data
...
\begin{itemize}
  \item Shine light on a metal surface; sometimes electrons are ejected
  \item Apply a \emph{stopping potential} $V_0$ to  halt just the
    fastest electrons
\end{itemize}
...
\medskip
\begin{block}{Einstein's hypothesis class}
\[
  V_0 = \frac{h}{e}\,\nu - \frac{W}{e}
\]
\begin{itemize}
  \item Affine in frequency
  \item Slope $= h/e$ is \textbf{universal} across all metals
  \item Hypothesized before  reliable $V_0$-vs-$\nu$ measurements available
\end{itemize}
\end{block}
...
%----------------------------------------------------------------------
\begin{frame}{Einstein's Inductive Bias}
...
\bigskip
Human intelligence (HI)  posited a low-parameter functional form.
...
\begin{itemize}
  ...
  \item Shifts intercept; \textbf{does not bias slope} if constant within an epoch
  \end{itemize}
...
\bigskip
Computing  regression is trivial if you have good data.

\bigskip
\textbf{29-year gap between Einstein and Millikan comes from  data engineering process, not statistical
re-specification.}
...
\medskip
Having a  wide frequency range helps (Cram\'er--Rao bound):
\[
...
\begin{itemize}
  ...
  \item Data span nearly a factor of two in frequency
\end{itemize}

\end{frame}

%----------------------------------------------------------------------
\begin{frame}{Estimating Planck's Constant}
...
\[
  (\hat\alpha_{\mathrm{MLE}},\hat\beta_{\mathrm{MLE}}) =
...
\bigskip
\begin{itemize}
  \item Confirming a regression equation says little about the physical structure  that generated it
  \item Compton scattering (1923) finally convinced him
  \item Millikan's 1923 Nobel prize: ``\ldots first direct experimental proof
...
\begin{itemize}
  ...
  \item Deduced  from $E = h\nu$
  \item Five training points are more than sufficient
\end{itemize}

\medskip
What a DL ML system misses:
\begin{enumerate}
  \item Does not  identify $\hat\beta = 4.129\times10^{-15}$ as $h/e$
  \item Does not  impose $\beta_{\mathrm{Na}} = \beta_{\mathrm{Li}}$
    from first principles
  \item Does not  decompose intercept into $W/e$ vs.\ $v_c$
  \item Does not  reduce data requirement from $O(10^3)$ to 5
\end{enumerate}
...
five training points sufficient (not more data, not a better algorithm)
...
\bigskip
Modern AI systems  \emph{induce} a linear law from lots of data and
search over many parameters of a DNL.

Einstein's HI system  deduced  an affine law (no data?)
