...
here.
Dave  insisted on connecting theory tightly to data. More important, Dave was extraordinarily generous as a colleague and teacher at NYU Stern.
I thank  Jonathan Payne, Jiao Shi, and B\'alint Sz\H{o}ke and for helpful discussions that preceded my giving the Backus lecture.}}
...
Long-standing puzzles about the term structure of interest rates motivated some of Dave Backus's important work. They motvate this paper too.  These puzzles are about the joint distribution of macroeconomic variables and asset returns implied by  some popular benchmark macroeconomic models.   The nominal yield curve usually slopes upward, the long-minus-short yield spread narrows before recessions and widens
after them, and---most strikingly---long-term bond yields are nearly
as volatile as short-term yields even though, under the
rational-expectations expectations theory of the term structure, they
should be much smoother averages of expected future short rates.  This
is Robert Shiller's (1979) ``volatility puzzle.''
Sargent~(1972, 1979) had tested the cross-equation restrictions on
vector autoregressions implied by the rational expectations theory of
the term structure, finding support for the theory's implications for
conditional means but early hints of the difficulties with conditional
second moments that Shiller later codified.  To resolve the volatility
puzzle, risk prices---or something observationally equivalent---must be
large, volatile, and state-dependent.  What generates those state-dependent risk prices? Risk aversion? Wrong beliefs? Fears
of model misspecification?   Disentangling potential  sources of those puzzling risk prices is the topic this paper.
...
higher risks.  But if the representative investor holds wrong beliefs,
observed average returns depend on \emph{both} risk aversion \emph{and}
misunderstood return distributions.  Wrong beliefs can contribute what from the point of view of an econometrician's baseline model 
look like ``stochastic discount factor shocks'' that have the  potential to 
explain observed countercyclical risk prices.
...
The ``three martingales'' of the sub title are the cumulative likelihood
...
The rest of the paper proceeds as follows.  Section~\ref{sec:risks-and-beliefs}
sets up the Gaussian framework in which a likelihood ratio twists
...
Section~\ref{sec:identification} discusses the identification challenge:
the same likelihood ratio can be interpreted as risk aversion or as
belief distortion. Section~\ref{sec:PSS} describes the approach of
...
Section~\ref{sec:robust} develops the robust control theory approach,
beginning with Hansen's formulation with a discounted entropy ball and
culminating in Hansen and Sz\H{o}ke's tilted entropy ball that
accommodates concerns about parametric alternatives with more long-run
risk.  Section~\ref{sec:empirics} summarizes empirical results.
Section~\ref{sec:conclusion} concludes with reflections on why these
joint probability distributions matter for macroeconomics.
...
\end{align}
where $y_{t+1}$ collects utility-relevant variables,
$r_t = \bar r\, x_t$ is the risk-free one-period interest rate, and
$d_t = \bar d\, x_t$ is the payout process from an asset.  The object
of interest is the price at date~$t$ of a claim to the random payout
stream $\{d_{t+j}\}_{j=1}^\infty$.

\begin{figure}[t]
\centering
\includegraphics[width=\linewidth,keepaspectratio]{fig2_tom.pdf}
\caption{The econometrician's model: estimated state dynamics.}
...
(Hicks--Shiller) prices a zero-coupon risk-free claim to one dollar at
time $t+n$ as:
\begin{align}
p_t(1) &= \exp(-r_t), \notag\\
...
With this device, modern asset pricing (Lucas--Hansen for stocks,
Dai--Singleton--Backus--Zin for bonds) takes the form:
\begin{align}
...
PSS find $w^* \neq 0$:
\begin{itemize}
\item The expert's model differs systematically from the
      econometrician's.
...
\[
   m_{t+1} := \exp\!\left(-\tfrac{w_t' w_t}{2}
       - w_t'\, \varepsilon_{t+1}\right),
\]
...
problem:
\begin{align}
J(x_0, c_0 \mid \eta) &:= \min_{\{w_t\}_{t\geq 1}}
...
\subsection{Yaron's bazooka}

Bansal and Yaron~(2004) mix long-run risk (LRR) with Epstein--Zin (EZ)
preferences.  LRR is statistically difficult to detect and estimate, but
...
&\quad\hphantom{\text{s.t.}}\quad
    ...
    - {\color{orange} w_t}). \notag
\end{align}
The worst-case shock mean distortion is
\[
   \tilde w_t = \tilde w\, x_t,
\]
...
\item $\bar w_t$: Sz\H{o}ke's especial LRR parametric worry;
...
each component---is therefore essential for both positive and normative
macroeconomics.
...
\[
   m_{t+1}^* \propto \exp\!\left(
       -\frac{W(x_{t+1}, c_{t+1})}{\theta}\right).
\]
